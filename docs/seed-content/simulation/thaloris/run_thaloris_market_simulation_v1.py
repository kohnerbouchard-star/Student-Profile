...
C=json.loads((ROOT/'input-v1.json').read_text())
...
with (OUT/'instrument-results.csv').open('w',newline='') as f:
  w=csv.DictWriter(f,fieldnames=['scenario','seed','instrumentId','symbol','return']); w.writeheader(); w.writerows(rows)
with (OUT/'portfolio-results.csv').open('w',newline='') as f:
  w=csv.DictWriter(f,fieldnames=['scenario','seed','strategy','return']); w.writeheader(); w.writerows(prows)
s={'simulationId':C['simulationId'],'seeds':C['seeds'],'cycles':C['cycles'],'scenarioStrategySummary':{},'integrity':{}}
