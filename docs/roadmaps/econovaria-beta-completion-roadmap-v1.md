...
- **`BETA-ADMIN-UI-V2-001` — Source-owned Admin v2 foundation and Overview reference migration**
  - Status: `IN_PROGRESS`
  - Owner branch and base SHA: `refactor/admin-ui-v2-overview-foundation-v1`, reconciled onto fetched `origin/main` at `f2694e40bac39b4ceb20951d88dddd38c6a9270a`. The product owner authorized committing and publishing this bounded Phase 1 implementation, opening one draft pull request, and obtaining a non-production Vercel preview; merge and production deployment remain unauthorized.
  - Ownership reconciliation: no open pull request owns the Admin v2 architecture migration. Draft PR #498 / `fix/admin-modal-focus-order-v1` owns only the legacy modal focus-order compatibility correction (`BETA-ADMIN-MODAL-A11Y-008`). The retained `frontend/admin-terminal-source-v1` branch is a stale source-preservation donor without a pull request, not active feature authority, and must not be merged wholesale.
  - Scope implemented locally: isolated source-owned vanilla-ESM Admin shell and component system; canonical primary navigation with World Management under the first-class `World` group; accepted v606 visual language as the design baseline; authoritative Overview reads and all six data states; explicit two-step v2-to-legacy route boundary for every other destination; shared tokens, dialog/drawer, data, form, table, error, permission, and accessibility primitives.
  ...
  - Next exact roadmap item: publish and review the bounded Phase 1 draft and its preview, then stop. After separately authorized Phase 1 completion, the approved migration sequence is Store plus shared artwork/media resolution; World Management as a native v2 route; Players and Attendance; Contracts; Marketplace, Settings, and Logs; then legacy deletion. Do not begin those phases automatically.
