...
- [x] `BETA-ADMIN-003` Remove remaining obsolete credential-dialog renderer if still present. `VERIFIED_COMPLETE` through PR #226: the access-code bridge is event-only, dead suppression code was removed from Player create UX, and the source ratchet prevents the legacy marker or renderer from returning.
