...
Current tranche (2026-08-18): `ARCH-100C — Player Inventory context propagation` is owned solely by `refactor/player-inventory-context-propagation-v1`, created from exact verified `main` `c4710c0a7fc51773f8a1fb0050e6b2d1b785d3aa`. Open PRs #619, #620, #624 and #626 have no exact overlap with the bounded backend Inventory scope. Six stale `feat/economic-asset-ownership-core-v2-*` aliases overlap five Inventory read files but are non-authoritative donor hazards from already-merged PR #503 and will not be merged or cherry-picked. The tranche excludes root manifests, workflows, Player Terminal/realtime/browser files, the singleton Player authority manifest, migrations/RPC SQL, Admin files, UI and deployment files. It will add a dependency-neutral Inventory context type port over the shared canonical context, preserve the exact frozen Player context through both handlers and explicit read/redemption use-case seams into their repositories, and allow scalar projection only inside the Supabase persistence adapters. Characterized direct handler callers will receive a server-generated canonical context; unsupported/malformed operations remain scope-free. Parsed redemption idempotency stays separate and unchanged. The current Supabase changelog was reviewed: no Edge, Data API or database breaking change applies because this tranche changes no runtime dependency, nested function call, table exposure, schema or RPC signature. Implementation and tests are pending on the owner branch; `ARCH-101` remains dependency-blocked.
