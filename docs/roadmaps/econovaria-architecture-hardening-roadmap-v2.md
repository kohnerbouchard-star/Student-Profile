...
Implemented, not merged (2026-08-18): `ARCH-100C — Player Inventory context propagation` is owned solely by `refactor/player-inventory-context-propagation-v1`, created from and still merge-based exactly on verified `main` `c4710c0a7fc51773f8a1fb0050e6b2d1b785d3aa`. The refreshed collision audit covers open PRs #619, #620, #624 and #626; none overlaps the bounded backend Inventory paths. Six stale `feat/economic-asset-ownership-core-v2-*` aliases overlap five read files but remain non-authoritative donor hazards from merged PR #503 and were not merged or cherry-picked. Root manifests, workflows, Player Terminal/realtime/browser files, the singleton Player authority manifest, migrations/RPC SQL, Admin files, UI and deployment files remain excluded.
...
Local evidence passes: Player Inventory 50/50; Player request-scope/privacy 22/22; Player security and both-root composition 43/43; adjacent Crafting 25/25, Store 14/14 and Marketplace 48/48; Crafting runtime 37/37; economic/store authority contracts 16/16; auth boundaries 16/16 plus 8/8; `typecheck:all`; the separate Player API Deno check; full backend smoke including Admin API 160/160; the complete root `npm test`; high-priority, architecture and legacy ratchets; deterministic inventory; JSON, secret and diff checks. The initial root composite attempt exposed only missing lockfile-declared local test dependencies; `npm ci --ignore-scripts` restored them without a tracked change, the exact web-session suite passed 45/45, and the full composite rerun passed. Independent review found no correctness, security, privacy, dependency-direction or behavior-parity defect. No route, RPC, schema, migration, runtime dependency, response, UI, deployment, economic-write or idempotency behavior changed; tranche-specific connected staging is inapplicable, while exact-head and exact-merge automated evidence remains mandatory. The current Supabase changelog was reviewed and no Edge, Data API or database breaking change applies. Draft PR #637 owns delivery. `ARCH-100C` is `IMPLEMENTED_NOT_MERGED`; exact-head CI, merge and exact-merge evidence are pending, so `ARCH-100` remains `IN_PROGRESS` and `ARCH-101` remains dependency-blocked.

Named remaining tranches are dependency-serialized. `ARCH-100C` is the current exact item and must merge with its full acceptance evidence before `ARCH-100D — Admin Game Sessions context propagation` begins. `ARCH-100D` must use a newly audited owner branch from then-current merged `main` to pass reviewed Admin context through the existing Game Sessions join-code/settings application and repository paths across both Admin and Classroom caller families. Re-audit residual scalarized paths after each tranche and name any additional bounded owner before `ARCH-100` can close. `ARCH-101` must not start first.
