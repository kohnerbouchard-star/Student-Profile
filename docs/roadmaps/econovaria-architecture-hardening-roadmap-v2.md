...
Unresolved acceptance boundary: `ARCH-100D` is `IMPLEMENTED_NOT_MERGED`, not complete. PR #638 must pass exact-head review/checks, merge into `main`, and pass exact-merge deployment inventory, staging/production parity, production release/health and branch-hygiene evidence. `ARCH-100` remains `IN_PROGRESS` and `ARCH-101` remains blocked. The next exact item after that merge gate is `ARCH-100E — residual context propagation classification and owner split`: begin with the multi-game Admin/Staff bootstrap reads in `admin-api/common.ts` and `staffBootstrapHttpHandler.ts`, then classify scalarized Admin Players/Attendance/Store/Contracts paths, remaining Admin domain routers, shared non-Game-Sessions Staff/Classroom handlers and Player non-Inventory domains into collision-bounded owners. None of those paths is claimed complete by `ARCH-100D`.
