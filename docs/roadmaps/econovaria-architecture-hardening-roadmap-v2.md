...
Execution record (2026-08-17): Phase 0 is merged and verified through `92a818dfd719d1dcf1407c39bb6904f5d9b78077`. The existing owner branch for the first bounded tranche is `refactor/request-application-context-v1`, created from that exact `main` and reconciled with current `main` `87cb22820db4120668a860e9fd9ebcbc02779c10` after PR #632 merged. The live open-PR collision audit covers #619 (root dependency manifests), #620 (GitHub workflows), #624 (Player Terminal CSS/realtime/browser tests), and #626 (Business browser acceptance); none owns the Player request-scope, rate-limit dispatcher, Inventory handler, or Player/Classroom composition files in this tranche. #624 also changes the singleton PR-bound Player authority manifest required by CI; at audited head `83a9d2af37d2eab3202eee7293780dbd31443edb` it is dirty/unmergeable with four failed checks, so #633 is serialized first and #624 must rebase/rebind that procedural manifest afterward. PR #632's release workflows, release contract, and `vercel.json` were audited before merge and do not overlap. `ARCH-100A` now consolidates the live `PlayerRequestScope` equivalent into an immutable internal Player application context with server-generated request identity, then forwards that same server-derived object from the reviewed rate-limit boundary into Inventory read/redemption handlers in both equivalent entrypoints. Unsupported methods retain scope-free handler behavior, direct handler callers retain a characterized fallback during the transition, and a composition contract proves both roots pass the context; no browser contract receives it. Implementation files are `backend/src/shared/requestApplicationContext.ts`, the Player scope and central rate-limit dispatcher, Inventory read/redemption handlers, both Player composition roots, focused tests, `backend/package.json`, the exact-path Player authority manifest, the generated architecture inventory, and both roadmaps. Local evidence passes: Player request-scope/privacy 22 tests, Player security/rate limiting 43 tests, Player Inventory 46 tests, Player cross-cutting authority 5 tests plus exact 17-changed/19-allowed-path verification, the full backend smoke suite (including 157 Admin API tests), backend `typecheck:all`, explicit Player API Deno check, auth boundaries (16 + 8), high-priority boundary ratchet (80), Admin architecture ratchet, legacy-runtime audits, architecture v2 ratchet at 168 cross-domain imports, repository secret scan, and `git diff --check`. No route, RPC, migration, schema, response, UI, deployment, economic-write, or idempotency behavior changes; connected staging evidence is not required for this internal refactor. Draft PR #633 carries implementation commit `cb356a8fb6e6d6f5b826a437b60d33c6121d1429` and PR-bound authority commit `5592fb369cd4d2c195a41952c25d9ff265634932`; required full-PR evidence is pending. Admin authentication/capability policy remains distinct: the next bounded tranche will project an Admin context only after the existing guard and owned-game check. The item remains `IN_PROGRESS` and must not be treated as complete after the Player tranche alone.
