...
Candidate `552b2e045068279bb9d2c88b9854c13a3f059b24` passed 50 workflows with one expected staging-only skip. The remaining multiplayer/load workflow failed before fixture creation because its disposable Edge gateway returned 500/502/503 during the short startup probes. The workflow now invokes the existing bounded, localhost-only `restartLocalEdgeRuntime` helper before those strict probes, as the other connected suites already do. A permanent source regression binds this ordering; no timeout, journey, load profile, assertion or enforcement step is removed.
