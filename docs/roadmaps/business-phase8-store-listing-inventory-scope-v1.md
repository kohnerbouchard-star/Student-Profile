...
9. Increment the offer version exactly once for a newly committed stock command while keeping quantity authority exclusively in Inventory.
10. Return only public keys and bounded commercial/custody state.
11. Add typed Store-domain contracts and repository support for the service command.
...
- Matching retries cannot move units twice or advance the offer twice.
...
4. Finished Goods account and item holding;
5. canonical Inventory transaction/posting;
6. offer version/public receipt state.

A matching committed idempotent transaction is checked before rejecting a retry for the now-advanced offer version. Competing new commands still require the current offer version and sufficient unreserved Finished Goods.
...
- forward-only migration(s) for canonical poster compatibility, account invariants, stock command, and schema assertions;
...
Implement the bounded canonical-poster extension, deterministic offer-scoped account authority, exact Finished Goods-to-Store transfer command, typed Store contracts, tests, and dedicated CI. Do not widen checkpoint 8A into withdrawal or buyer settlement.
