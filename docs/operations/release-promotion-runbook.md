...
from `main`. Staging and production consume the same GitHub Actions artifact ID,
release manifest, archive bytes, and `artifactSetSha256`. Promotion never invokes
the release builder.
...
3. The workflow verifies the commit is reachable from `origin/main`, checks out
   that exact commit, builds deterministic `tar.gz` archives, and generates:
...
4. Record the workflow run ID, immutable artifact ID, artifact digest,
   `releaseId`, commit, and `artifactSetSha256`.
5. Do not rerun the builder to replace an existing release. A correction requires
...
7. Record deployed function versions, frontend release SHA, deployment time,
   operator, target identities, and the exact artifact-set digest.
...
Production promotion is prohibited unless the same `releaseId` and
`artifactSetSha256` have passing staging Player and Admin evidence.

1. Create a production promotion record referencing the staging evidence and a
   previous immutable production release as the rollback target.
...
4. Deploy through the approved production adapters without rebuilding or editing
...
1. Identify the prior known-good build workflow run ID, artifact ID, release
   manifest, commit, and `artifactSetSha256`.
...
5. Verify checksums, visible release SHA, function versions, critical journeys,
   and recovery timing.
...
- configuration version or feature flags differ;
- production lacks same-artifact staging Player/Admin evidence;
