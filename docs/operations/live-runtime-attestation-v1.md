...
2. Edge Function Inventory reports `PASS` for exact inventory, `verify_jwt`, and source-digest parity.
3. The exact current `main` commit builds as a staged production deployment with no production-domain alias.
4. `/api/health` reports both Staff and Player session boundaries ready and bound to the production Supabase project.
5. The staged deployment has no error logs and passes an HTTP timing probe.
6. A protected production-environment workflow promotes that exact deployment.
7. The promoted production endpoint repeats the health and error-log checks.
...
The Release Integrity artifact must contain a `PASS` attestation whose `sourceSha` equals the requested SHA. The Edge Function artifact must have `ok: true`. Artifacts from a different commit cannot be used.
...
3. Deploys a production-targeted artifact with `--skip-domain`.
4. Verifies `/api/health` on the unaliased deployment.
5. Inspects the deployment, runs `httpstat`, and rejects error logs.
6. Promotes the verified deployment.
7. Repeats health and error-log checks on production.

The workflow never uses an unverified Git-generated production deployment.

## Runtime health contract

`/api/health` validates the configured environment, exact Supabase project ref, and exact project URL. It then probes:

- `web-session-api/health`
- `player-web-session-api/health`

It returns `ready` only when both boundaries return a bounded successful health payload. It exposes no secret values, tokens, user data, database information, or internal error details.
...
Vercel promotion is atomic at the routing layer. If post-promotion health fails, the operator must restore the previously verified Vercel deployment rather than rebuild from another source. Supabase and database changes are controlled by their own candidate/promotion workflows and are not rolled back by Vercel deployment movement.
