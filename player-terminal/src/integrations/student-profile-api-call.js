...
const READ_MODEL_KEYS = new Set([
  "countries", "news", "market", "marketAsset", "portfolio",
  "store", "banking", "notifications"
]);

function normalizedBaseUrl(value) { return String(value || "").trim().replace(/\/+$/, ""); }
...
function endpointProjection(snapshot, endpointKey) { return endpointKey === "marketAsset" ? snapshot.market : snapshot[endpointKey]; }
function responseCode(body) { return String(body?.code || body?.error?.code || "REQUEST_FAILED").toUpperCase(); }
...
export function createStudentProfileApiCall({ request } = {}) {
  ...
  async function loadCapabilityManifest(context) {
    ...
    if (!route) {
      throw new ApiConnectionPendingError({ endpointKey: "capabilities", method: "GET", path: "/capabilities" });
    }
