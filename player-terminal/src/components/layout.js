...
const PLAYER_ROUTE_DOMAINS = Object.freeze({
  dashboard: "session",
  profile: "session",
  news: "world",
  market: "market",
...
function renderNavGroup(group, route, data) {
  ...
  return `<div class="player-terminal-nav-group${active ? " is-active" : ""}">
...
export function renderShell({ route, data, pageHtml, ui, config }) {
  return `<div class="player-terminal-overview player-terminal-app-root">
    ...
    ${renderMobileNavigation(route, data, ui.mobileMenuOpen)}
  </div>`;
}
