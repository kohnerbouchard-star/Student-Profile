...
export function renderBusinessPage(data) {
  ...
  if (business?.configured === false) {
    return `<section class="player-terminal-page player-terminal-business-page"><div class="player-terminal-page-heading"><div><small>PLAYER ENTERPRISE</small><h2>Business</h2><p>Operate a player-owned company when the business service is enabled for this game.</p></div></div>${renderEmptyState({ title: "Business is not configured", detail: "Company profiles, production, products, and suppliers will appear after the business domain is connected.", iconName: "business" })}</section>`;
  }
