...
export function renderMarketplacePage(data, ui) {
  ...
  if (market?.configured === false) {
    return `<section class="player-terminal-page player-terminal-marketplace-page"><div class="player-terminal-page-heading"><div><small>PLAYER COMMERCE</small><h2>Marketplace</h2><p>Trade with other players when the marketplace service is enabled.</p></div></div>${renderEmptyState({ title: "Marketplace is not configured", detail: "Listings, settlement, and seller tools will appear after the marketplace domain is connected.", iconName: "marketplace" })}</section>`;
  }
