...
export function renderDashboardPage(data) {
  ...
  const hasSavings = dashboard.savingsBalance !== null && dashboard.savingsBalance !== undefined && Number.isFinite(Number(dashboard.savingsBalance));
  const liquidFunds = Number(dashboard.liquidBalance) + (hasSavings ? Number(dashboard.savingsBalance) : 0);
      ...
      <section class="player-terminal-panel player-terminal-finance-snapshot">
        ...
        <div class="player-terminal-wealth-snapshot">
          <div class="player-terminal-wealth-breakdown">
            <button type="button" data-route="banking"><span>${icon("banking")}</span><div><small>${hasSavings ? "Cash & savings" : "Available cash"}</small><strong>${escapeHtml(formatCurrency(liquidFunds, currencyCode))}</strong></div></button>
