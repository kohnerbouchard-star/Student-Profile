...
export function renderProgressionPage(data, ui) {
  ...
  if (progression?.configured === false) {
    return `<section class="player-terminal-page player-terminal-progression-page"><div class="player-terminal-page-heading"><div><small>PLAYER DEVELOPMENT</small><h2>Progression</h2><p>Track levels, skills, achievements, and licenses when progression is enabled.</p></div></div>${renderEmptyState({ title: "Progression is not configured", detail: "XP, skills, achievements, and licenses will appear after the progression domain is connected.", iconName: "trophy" })}</section>`;
  }
