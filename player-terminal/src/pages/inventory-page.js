...
function renderInventoryItem(item, currencyCode) {
  const usable = item.category === "Consumables" || item.category === "Access";
...
export function renderInventoryPage(data, ui) {
  ...
  const capacityPercent = Math.round((data.inventory.capacityUsed / data.inventory.capacityMax) * 100);
  ...
  return `<section class="player-terminal-page player-terminal-inventory-page" data-page="inventory">
    <header class="player-terminal-page-heading">
      ...
      <div class="player-terminal-capacity-meter"><div><small>STORAGE CAPACITY</small><strong>${escapeHtml(data.inventory.capacityUsed)} / ${escapeHtml(data.inventory.capacityMax)}</strong></div><span><i style="width:${capacityPercent}%"></i></span></div>
    </header>
