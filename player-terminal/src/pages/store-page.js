...
export function renderStorePage(data, ui) {
  ...
  return `<section class="player-terminal-page player-terminal-store-page" data-page="store">
    ...
    <section class="player-terminal-panel player-terminal-connection-note">
      <div>${icon("globe")}<span><strong>Store purchase connector</strong><small>Purchase buttons request a server quote, complete an idempotent purchase, and refresh authoritative inventory.</small></span></div>
      ${renderStatusPill("ADAPTER READY", "cyan")}
    </section>
  </section>`;
}
