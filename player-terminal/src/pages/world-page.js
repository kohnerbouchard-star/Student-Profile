...
function renderLocationAndRoutes(model, quote, capabilities) {
  ...
  return `<section class="player-world-panel player-world-map-panel" aria-labelledby="world-travel-title"><header><div><small>GEOGRAPHY & TRAVEL</small><h2 id="world-travel-title">${escapeHtml(title(state?.currentLocationId, "Location unavailable"))}</h2></div>${statusPill(title(state?.status, "Unavailable"), state?.status === "available" ? "good" : "warn")}</header>
    ...
    ${activeJourney ? `<article class="player-world-journey" role="status"><div><small>ACTIVE JOURNEY</small><strong>${escapeHtml(activeJourney.fromLocationId)} → ${escapeHtml(activeJourney.toLocationId)}</strong><p>${escapeHtml(activeJourney.totalDurationMinutes)} minutes · ${escapeHtml(formatCurrency(activeJourney.totalCostMinor, activeJourney.currencyCode))}</p></div><form data-world-form="travelComplete" data-journey-id="${escapeHtml(activeJourney.publicJourneyId)}"><button class="player-terminal-primary-button" type="submit" ${canComplete ? "" : "disabled"}>Complete eligible arrival</button></form></article>` : ""}
    ...
    <div class="player-world-quote" aria-live="polite">${quote ? `<small>AUTHORITATIVE QUOTE</small><strong>${escapeHtml(quote.fromLocationId)} → ${escapeHtml(quote.toLocationId)}</strong><p>${escapeHtml(formatCurrency(quote.totalCostMinor, quote.currencyCode))} · ${escapeHtml(quote.totalDurationMinutes)} minutes</p><ol>${(quote.legs || []).map((leg) => `<li>${escapeHtml(title(leg.mode))}: ${escapeHtml(leg.fromLocationId)} → ${escapeHtml(leg.toLocationId)}</li>`).join("")}</ol><form data-world-form="travelExecute"><input type="hidden" name="quoteId" value="${escapeHtml(quote.publicQuoteId)}"><button class="player-terminal-primary-button" type="submit" ${canExecute ? "" : "disabled"}>Confirm travel</button><p class="player-world-form-status" data-world-form-status aria-live="polite"></p></form>` : emptyState("No active quote", "Select a destination and eligible travel modes. Quotes expire and are revalidated at execution.")}</div></div>
