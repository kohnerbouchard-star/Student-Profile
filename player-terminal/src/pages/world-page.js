...
function renderArrival(arrival, capabilities) {
  ...
  if (!arrival?.required) {
    return `<section class="player-world-panel" aria-labelledby="world-arrival-title"><header><div><small>ARRIVAL CLASS</small><h2 id="world-arrival-title">No Arrival Class</h2></div>${statusPill("Not required", "neutral")}</header>${emptyState("No questionnaire configured", "This game does not require an Arrival Class questionnaire. World, travel, and residency features remain available.")}</section>`;
  }
  if (!questionnaire) {
    return `<section class="player-world-panel" aria-labelledby="world-arrival-title"><header><div><small>ARRIVAL CLASS</small><h2 id="world-arrival-title">Questionnaire unavailable</h2></div>${statusPill("Configuration required", "warn")}</header>${emptyState("Arrival Class cannot be completed", "This game requires an Arrival Class, but no questionnaire has been published.")}</section>`;
  }
