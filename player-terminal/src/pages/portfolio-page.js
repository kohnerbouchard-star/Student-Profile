...
export function renderPortfolioPage(data) {
  ...
  const hasSavings = data.dashboard.savingsBalance !== null && data.dashboard.savingsBalance !== undefined && Number.isFinite(Number(data.dashboard.savingsBalance));
  const liquidFunds = Number(data.dashboard.liquidBalance) + (hasSavings ? Number(data.dashboard.savingsBalance) : 0);
  ...
  return `<section class="player-terminal-page player-terminal-portfolio-page" data-page="portfolio">
    ...
    <div class="player-terminal-portfolio-metrics">
      ...
      <article class="is-amber"><span>${icon("wallet")}</span><div><small>LIQUID FUNDS</small><strong>${escapeHtml(formatCurrency(liquidFunds, currencyCode))}</strong><em>${hasSavings ? "Checking + savings" : "Available cash"}</em></div></article>
    </div>
