...
function idempotencyKey(payload, endpointKey, contextKey = "") {
  return requiredText(
    payload?.idempotencyKey || contextKey,
    "idempotencyKey",
    endpointKey
  );
}
...
function publicStoryDeliveryId(value) {
  const deliveryId = requiredText(value, "deliveryId", "storyDeliveryState")
    .toLowerCase();
  if (/^ndl_[0-9a-f]{32}$/.test(deliveryId)) return deliveryId;
  throw new ApiRequestError(
    "deliveryId is invalid for storyDeliveryState.",
    { body: { code: "player_story_delivery_id_invalid" } }
  );
}

function storyDeliveryAction(value) {
  const action = requiredText(value, "action", "storyDeliveryState")
    .toLowerCase();
  if (["seen", "dismissed", "acknowledged"].includes(action)) return action;
  throw new ApiRequestError(
    "action is invalid for storyDeliveryState.",
    { body: { code: "player_story_delivery_action_invalid" } }
  );
}
...
function notificationDeliveryIds(payload, endpointKey) {
  ...
  throw new ApiRequestError(
    ...
    {
      body: {
        code: "player_notification_delivery_ids_invalid",
        endpointKey
      }
    }
  );
...
const ROUTE_BUILDERS = Object.freeze({
  ...
  marketAsset: ({ params = {}, payload = {} }) => ({
    method: "GET",
    path: queryPath(
      `/players/me/stocks/assets/${encodeURIComponent(requiredText(
  ...
  marketOrder: ({ payload = {}, idempotencyKey: contextKey }) => {
    ...
    return {
      ...
      payload: {
        ...
        idempotencyKey: idempotencyKey(payload, "marketOrder", contextKey)
      }
    };
  },
  marketWatchlist: ({ params = {}, payload = {} }) => {
    if (typeof payload.enabled !== "boolean") {
      throw new ApiRequestError(
        ...
        { body: { code: "player_watchlist_state_invalid" } }
      );
    }
    return {
  ...
  storePurchase: ({ payload = {}, idempotencyKey: contextKey }) => ({
    method: "POST",
    path: "/players/me/store/purchases",
    payload: {
      quoteKey: requiredText(payload.quoteKey, "quoteKey", "storePurchase"),
      idempotencyKey: idempotencyKey(payload, "storePurchase", contextKey),
  ...
  inventoryUse: ({
    ...
    payload: {
      ...
      idempotencyKey: idempotencyKey(payload, "inventoryUse", contextKey)
    }
  }),
  banking: ({ payload = {} }) => ({
    method: "GET",
  ...
  progression: () => ({
    method: "GET",
    path: "/players/me/progression"
  }),
  progressionUnlock: ({ params = {}, payload = {}, idempotencyKey: contextKey }) => ({
    method: "POST",
    path: `/players/me/progression/skills/${encodeURIComponent(requiredText(
      params.skillId || payload.skillId,
      "skillId",
      "progressionUnlock"
    ))}/unlock`,
    payload: {
      idempotencyKey: idempotencyKey(payload, "progressionUnlock", contextKey)
    }
  }),
  progressionClaim: ({ params = {}, payload = {}, idempotencyKey: contextKey }) => ({
    method: "POST",
    path: `/players/me/progression/rewards/${encodeURIComponent(requiredText(
      params.rewardId || payload.rewardId,
      "rewardId",
      "progressionClaim"
    ))}/claim`,
    payload: {
      idempotencyKey: idempotencyKey(payload, "progressionClaim", contextKey)
    }
  }),
  ...
  storyDeliveryState: ({ params = {}, payload = {} }) => ({
    method: "POST",
    path: `/players/me/story-deliveries/${encodeURIComponent(publicStoryDeliveryId(
  ...
  logout: () => ({
    method: "POST",
    path: "/players/me/session/logout"
  })
});
