export const PLAYER_ENDPOINTS = Object.freeze({
  ...
  storeQuote: { method: "POST", path: "/store/quote" },
...
export function resolveEndpoint(endpoint, params = {}) {
  const path = Object.entries(params).reduce(
    (path, [key, value]) => value === undefined || value === null || String(value).trim() === ""
      ? path
      : path.replace(`:${key}`, encodeURIComponent(String(value))),
    endpoint.path
  );
  const unresolved = path.match(/:([A-Za-z][A-Za-z0-9_]*)/);
  if (unresolved) {
    throw new TypeError(`Missing endpoint parameter: ${unresolved[1]}`);
  }
  return path;
}
