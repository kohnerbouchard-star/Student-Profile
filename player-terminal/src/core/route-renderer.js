...
import { renderBusinessWorkspacePage as renderConfiguredBusinessWorkspacePage } from "../pages/business-workspace-page.js";
...
export function renderBusinessWorkspacePage(data) {
  if (data?.business?.configured === true) {
    return renderConfiguredBusinessWorkspacePage(data);
  }
  return renderBusinessFormationPage(data);
}
...
function renderWorldRoutePage(data) {
  ...
  const loading = !model && (
    view.state === "loading" ||
    data?.resourceStatus?.worldRuntime?.state === "loading"
  );
