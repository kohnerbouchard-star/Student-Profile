...
function safeReadError(error) {
  if (Number(error?.status) === 429) {
    return "That conversation is being updated too quickly. Try again shortly.";
  }
  if (
    Number(error?.status) >= 500 || error?.code === "NETWORK_ERROR" ||
    error?.code === "OFFLINE"
  ) {
    return "The conversation opened, but its read status could not be updated yet.";
  }
...
export function installMessageReadFlow({ mount, terminal, config }) {
  ...
  if (!terminal || typeof terminal.getState !== "function") {
    throw new TypeError(
      "The message read flow requires an active player terminal.",
    );
  }
  ...
  async function markThreadRead(threadId) {
    if (
      destroyed || pending.has(threadId) || !PUBLIC_THREAD_ID.test(threadId) ||
      !capabilityEnabled()
    ) return;
    ...
    try {
      ...
      if (!destroyed) await terminal.refresh();
    } catch (error) {
      if (!dispatchInvalidSession(error, config) && !destroyed) {
        terminal.showToast?.(safeReadError(error), "amber");
      }
    } finally {
  ...
  function handleClick(event) {
    const control = event.target.closest?.(
      '[data-player-message-thread][data-player-message-unread="true"]',
    );
    if (!control) return;
    const threadId = String(control.dataset.playerMessageThread || "")
      .trim()
      .toLowerCase();
