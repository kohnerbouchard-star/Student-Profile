...
const compatIndex = stylesheetOrder.indexOf("player-terminal-shell-compat.css");
...
if (!(compatIndex >= 0 && compatIndex < dashboardIndex && dashboardIndex < skeletonIndex)) {
  throw new Error("Dashboard route ownership must load after shell compatibility and before state-specific styles.");
}

if (/!important\b/.test(dashboardCss)) {
  throw new Error("The Dashboard route owner must not introduce !important declarations.");
}
if (/font-size\s*:\s*(?:[0-9]|10)px\b/.test(dashboardCss)) {
  throw new Error("The Dashboard refresh must not introduce sub-11px fixed text.");
}

const dashboardStats = await stat(path.join(root, "css/routes/player-terminal-dashboard.css"));
if (dashboardStats.size > 36_000) {
  throw new Error(`Dashboard route CSS exceeded its 36 KB ownership budget: ${dashboardStats.size}.`);
}

for (const marker of [
  "player-terminal-dashboard-page",
  "player-terminal-map-hud",
  "player-terminal-map-legend",
  "player-terminal-map-footer",
  "player-terminal-map-home",
  "player-terminal-map-metrics",
  "HOME MARKET",
...
for (const marker of [
  ...
  ".player-terminal-command-map-panel",
  ".player-terminal-map-hud",
  ".player-terminal-map-footer",
...
for (const marker of [
  "modal.type === \"country\"",
  "player-terminal-country-modal",
  "player-terminal-country-indicators",
  "player-terminal-country-intel-grid",
  "player-terminal-country-related"
]) {
  if (!modalSource.includes(marker)) throw new Error(`Country intelligence contract is missing: ${marker}`);
}

const packageJson = JSON.parse(packageSource);
if (!packageJson.scripts?.["dashboard-refresh"] || !packageJson.scripts.verify.includes("dashboard-refresh")) {
  throw new Error("The Dashboard refresh contract must run in npm run verify.");
}

console.log(`Player Dashboard refresh passed: route-owned hierarchy, protected map interactions, home-market summary, country modal, and ${dashboardStats.size}-byte bounded CSS verified.`);
