...
const [indexSource, tokensSource, foundationSource, packageSource] = await Promise.all([
  read("index.html"),
  read("css/player-terminal-tokens.css"),
  read("css/player-terminal-foundation.css"),
  read("package.json")
]);
const retired = ["player-terminal-base.css", "player-terminal.css", "player-terminal-ux.css", "player-terminal-polish.css", "player-terminal-normalization.css", "player-terminal-shell-compat.css", "player-terminal-route-compat.css"];
...
const requiredOrder = [
  "player-terminal-tokens.css", "player-terminal-foundation.css", "routes/player-terminal-shared-layout.css", "routes/player-terminal-shared-cards.css",
  "routes/player-terminal-shared-lists.css", "routes/player-terminal-shared-states.css", "routes/player-terminal-shared-details.css",
  "routes/player-terminal-shared-responsive.css", "routes/player-terminal-shared-overlays.css", "routes/player-terminal-dashboard.css",
  "player-terminal-skeletons.css", "player-terminal-interior-v2.css", "player-terminal-finance-v2.css", "player-terminal-communications-v2.css",
  "player-terminal-economy-items-v2.css", "player-terminal-operations-v2.css", "player-terminal-world-v2.css"
];
let previousIndex = -1;
...
if (/!important\b/.test(tokensSource) || /!important\b/.test(foundationSource)) throw new Error("Player tokens and the core foundation must not contain !important declarations.");
...
for (const file of retired) {
  try { await access(path.join(root, "css", file)); } catch (error) { if (error?.code !== "ENOENT") throw error; }
}
