...
const [mapSource, dashboardSource, appSource, polishSource, foundationSource, compatSource, sharedRouteSources, routeCompatCss, dashboardCss, indexSource] = await Promise.all([
...
if (protectedSharedRouteMapSelector.test(routeCss) || protectedSharedRouteMapSelector.test(routeCompatCss)) {
  throw new Error("Shared route ownership crossed into protected map ownership.");
}
...
if (!sharedRouteFiles.every((file) => indexSource.includes(file)) || !indexSource.includes("css/player-terminal-route-compat.css")) {
  throw new Error("Shared route ownership is not loaded by index.html.");
}
...
console.log(`Player map protection passed: ${countryIds.length} regions, geometry, renderer, keyboard hook, shared-route isolation, route-owned chrome, and interaction safety preserved.`);
