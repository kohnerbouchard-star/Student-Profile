...
const required = [
  ...
  "src/api/backend-routes.js",
  "src/api/read-model.js",
  "src/api/http-transport.js",
...
for (const endpoint of ["business:", "marketplace:", "crafting:", "loans:", "messages:", "progression:", "progressionUnlock:", "storeQuote:"]) {
  if (!endpointSource.includes(endpoint)) throw new Error(`Required endpoint is missing: ${endpoint}`);
}

const backendRouteSource = await readFile(path.join(root, "src/api/backend-routes.js"), "utf8");
for (const marker of ["/players/me/game/dashboard", "/players/me/world/countries", "/players/me/world/news", "/players/me/stocks/assets", "/players/me/ledger", "/players/me/store/quote", "/players/me/inventory", "/players/me/session/logout", "/accept", "/submit"]) {
  if (!backendRouteSource.includes(marker)) throw new Error(`Required player backend route mapping is missing: ${marker}`);
}

const httpTransportSource = await readFile(path.join(root, "src/api/http-transport.js"), "utf8");
if (!httpTransportSource.includes('headers["x-player-session-token"]')) {
  throw new Error("Direct player HTTP requests must send x-player-session-token.");
}

const readModelSource = await readFile(path.join(root, "src/api/read-model.js"), "utf8");
for (const marker of ["normalizeTerminalBootstrap", "mergeTerminalRead", "capacity: null", "availableActions"]) {
  if (!readModelSource.includes(marker)) throw new Error(`Required production read-model behavior is missing: ${marker}`);
}
