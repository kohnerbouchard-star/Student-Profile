-- Country snapshot initializer conflict patch V1.
-- Fixes runtime ambiguity in initialize_country_economic_snapshots_for_game(...)
-- where the RETURNS TABLE column country_profile_id conflicted with the
-- ON CONFLICT column list. Use the named unique constraint instead.
...
comment on function public.initialize_country_economic_snapshots_for_game(uuid, timestamptz, text, jsonb) is
  'Creates initial real-time country_economic_snapshots for every active map country in one game session. Patched to use the named unique constraint for conflict handling so RETURNS TABLE column names cannot conflict with table column references.';
