...
create unique index if not exists stock_tick_archives_object_key_uq on private.stock_tick_archives(object_key);
create index if not exists stock_tick_archives_session_range_idx on private.stock_tick_archives(game_session_id, range_start, range_end);
create index if not exists stock_tick_archives_status_idx on private.stock_tick_archives(status, created_at);
...
create table if not exists public.stock_price_candles (
  ...
  open numeric not null check (open > 0), high numeric not null check (high > 0), low numeric not null check (low > 0), close numeric not null check (close > 0),
...
create index if not exists stock_price_candles_lookup_idx on public.stock_price_candles(game_session_id, stock_asset_id, timeframe, bucket_start desc);
...
create or replace function private.upsert_stock_price_candles(p_game_session_id uuid,p_range_start timestamptz,p_range_end timestamptz)
...
begin
  ...
  with frames as (select '5m'::text timeframe, interval '5 minutes' span union all select '1h', interval '1 hour'),
  base as (
    select t.game_session_id,t.stock_asset_id,f.timeframe,date_bin(f.span,t.created_at,timestamptz '2000-01-01 00:00:00+00') bucket_start,t.tick_index,t.price,t.volume,t.created_at
    from public.stock_price_ticks t cross join frames f
    where t.game_session_id=p_game_session_id and t.created_at>=p_range_start and t.created_at<p_range_end
  ), agg as (
    select game_session_id,stock_asset_id,timeframe,bucket_start,
      (array_agg(price order by created_at asc,tick_index asc))[1] open,max(price) high,min(price) low,
      (array_agg(price order by created_at desc,tick_index desc))[1] close,sum(volume)::bigint volume,count(*)::integer tick_count,
      min(tick_index)::integer first_tick_index,max(tick_index)::integer last_tick_index
    from base group by game_session_id,stock_asset_id,timeframe,bucket_start
  )
  insert into public.stock_price_candles(game_session_id,stock_asset_id,timeframe,bucket_start,open,high,low,close,volume,tick_count,first_tick_index,last_tick_index)
  select game_session_id,stock_asset_id,timeframe,bucket_start,open,high,low,close,volume,tick_count,first_tick_index,last_tick_index from agg
  on conflict (game_session_id,stock_asset_id,timeframe,bucket_start) do update set
    open=excluded.open,high=excluded.high,low=excluded.low,close=excluded.close,volume=excluded.volume,tick_count=excluded.tick_count,
...
declare v_archive private.stock_tick_archives%rowtype; v_matching bigint; v_deleted bigint;
begin
  ...
  select count(*) into v_matching from public.stock_price_ticks where game_session_id=v_archive.game_session_id and created_at>=v_archive.range_start and created_at<v_archive.range_end and tick_index between v_archive.min_tick_index and v_archive.max_tick_index;
  ...
  delete from public.stock_price_ticks where game_session_id=v_archive.game_session_id and created_at>=v_archive.range_start and created_at<v_archive.range_end and tick_index between v_archive.min_tick_index and v_archive.max_tick_index;
  ...
  insert into private.stock_tick_archive_state(game_session_id,last_archived_at,last_archived_tick_index) values(v_archive.game_session_id,v_archive.range_end,v_archive.max_tick_index)
  on conflict(game_session_id) do update set last_archived_at=greatest(private.stock_tick_archive_state.last_archived_at,excluded.last_archived_at),last_archived_tick_index=greatest(private.stock_tick_archive_state.last_archived_tick_index,excluded.last_archived_tick_index),updated_at=now();
  return v_deleted;
end; $$;
...
comment on table public.stock_price_candles is 'Compact 5-minute, hourly, and future daily OHLCV history retained in Postgres after raw stock ticks are archived.';
comment on function private.upsert_stock_price_candles(uuid,timestamptz,timestamptz) is 'Builds exact 5-minute and hourly OHLCV candles from one clock-aligned hour of raw ticks.';