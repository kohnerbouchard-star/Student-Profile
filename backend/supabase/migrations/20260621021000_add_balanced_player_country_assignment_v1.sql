-- Balanced player country assignment V1.
-- Creates players through one transactional RPC and assigns each new player
-- to an active country using weighted random selection favoring countries
-- with fewer active player assignments.
-- No immigration policy, country preference UI, events, or supply-chain logic.
...
comment on function public.create_player_with_balanced_country_assignment(
  ...
  'Creates an active player and assigns an active country in one transaction. The weighted random selection favors countries with fewer active assignments using (maxActiveAssignments - activeAssignments + 1)^2, ensuring randomized but even country distribution.';
