...
-- Browser roles receive no direct table policy or table privilege. The
-- authenticated classroom Edge Function resolves the custom player session
-- and performs explicitly scoped reads and mutations with its service client.
...
grant execute on function public.ensure_player_stock_watchlist_active_scope_v1()
to service_role;
...
comment on column public.player_stock_watchlist.stock_asset_id is
  'Active per-game stock asset selected by the authenticated player.';
comment on function public.ensure_player_stock_watchlist_active_scope_v1() is
  'Rejects watchlist rows unless both the player and stock asset are active inside the same game session.';
