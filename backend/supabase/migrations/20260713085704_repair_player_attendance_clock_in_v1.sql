-- Repair migration for environments whose migration ledger contains the
-- original attendance migration but whose attendance table/RPC drifted away.
...
drop trigger if exists set_player_attendance_records_updated_at
on public.player_attendance_records;
...
create index if not exists player_attendance_records_game_date_idx
on public.player_attendance_records (game_session_id, attendance_date desc);

create index if not exists player_attendance_records_player_date_idx
