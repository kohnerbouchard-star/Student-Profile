-- Business V2 Phase 8A: extend canonical Inventory posting for Business Store-listing custody.
...
-- Retain one canonical Inventory poster. Store-owned compatibility stock keeps its
-- existing store_items binding, while Business-owned Store-listing accounts are
-- validated against one same-game Business seller offer and never carry the
-- seeded Store compatibility store_item_id projection.
...
declare
  ...
  v_store_offer public.store_seller_offers%rowtype;
...
begin
  ...
  loop
    ...
    if (v_account.account_kind in ('warehouse','work_in_progress','finished_goods') and v_party.party_kind <> 'business')
      or (v_account.account_kind = 'store_stock' and v_party.party_kind not in ('store','business'))
    ...
    if v_account.account_kind = 'store_stock' and v_party.party_kind = 'business' then
      if v_store_item_id is not null then
        raise exception 'INVENTORY_TRANSACTION_BUSINESS_STORE_PROVENANCE_INVALID:%', v_line_number using errcode = 'P0001';
      end if;
      ...
      for share;
      if not found then
        raise exception 'INVENTORY_TRANSACTION_BUSINESS_STORE_SCOPE_MISMATCH:%', v_line_number using errcode = 'P0001';
      end if;
