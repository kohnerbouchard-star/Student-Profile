-- Canonical Player Crafting job creation and reservations V2.
-- Preserves the public RPC signature and deterministic recipe behavior.
...
begin
  ...
  if p_game_session_id is null
    ...
    or p_quantity not between 1 and 25
    or jsonb_typeof(p_substitutions) <> 'object'
    or coalesce(p_idempotency_key, '') !~ '^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$'
  then
    raise exception 'CRAFTING_JOB_REQUEST_INVALID' using errcode = 'P0001';
  end if;
  ...
  if found then
    if v_job.request_hash <> v_request_hash then
      raise exception 'CRAFTING_IDEMPOTENCY_CONFLICT' using errcode = 'P0001';
    end if;
    return jsonb_build_object(
      ...
      'completesAt', v_job.completes_at,
      'committed', true,
  ...
  if v_pack_id is null then
    raise exception 'CRAFTING_PACK_INACTIVE' using errcode = 'P0001';
  end if;

  select r.* into v_recipe
  from public.physical_economy_recipe_definitions r
  where r.pack_id = v_pack_id
    and r.recipe_key = p_recipe_key
    and r.status = 'active'
  for share;
  if not found then
    raise exception 'CRAFTING_RECIPE_UNAVAILABLE' using errcode = 'P0001';
  end if;

  select a.* into v_availability
  from public.game_session_recipe_availability a
  where a.recipe_id = v_recipe.id
    and a.game_session_id = p_game_session_id
  for share;
  if not found or not v_availability.enabled or v_availability.scarcity_band = 'unavailable' then
    raise exception 'CRAFTING_RECIPE_UNAVAILABLE' using errcode = 'P0001';
  end if;

  if not v_availability.unlocked_by_default and not exists (
    select 1
    from public.player_recipe_unlocks u
    where u.game_session_id = p_game_session_id
      and u.player_id = p_player_id
      and u.recipe_id = v_recipe.id
      and u.revoked_at is null
  ) then
    raise exception 'CRAFTING_RECIPE_LOCKED' using errcode = 'P0001';
  end if;
  ...
  if exists (
    ...
    join public.physical_economy_salvage_rules sr
      on sr.pack_id = v_pack_id
     and sr.equipment_item_key = o.item_key
     and sr.enabled
    join public.equipment_instances ei
      on ei.game_session_id = p_game_session_id
     and ei.player_id = p_player_id
     and ei.item_key = o.item_key
    join public.equipment_salvage_jobs sj
      on sj.equipment_instance_id = ei.id
     and sj.game_session_id = p_game_session_id
     and sj.player_id = p_player_id
     and sj.status = 'settled'
  ...
  v_input_multiplier := case v_difficulty
    when 'easy' then 0.9
    when 'hard' then 1.15
    when 'insane' then 1.3
    else 1
  end;
  v_duration_multiplier := case v_difficulty
    when 'easy' then 0.8
    when 'hard' then 1.25
  ...
  if cardinality(v_availability.country_codes) > 0
    and (v_country is null or not v_country = any(v_availability.country_codes))
  then
    raise exception 'CRAFTING_RECIPE_COUNTRY_UNAVAILABLE' using errcode = 'P0001';
  end if;

  v_failure_basis_points := case v_difficulty
    when 'hard' then 250
    when 'insane' then 500
    when 'moderate' then 100
    else 0
  end;
  ...
  v_snapshot := jsonb_build_object(
    ...
    'recipeVersion', v_recipe.pack_id,
    'difficultyKey', v_difficulty,
    'countryCode', v_country,
    'quantity', p_quantity,
    'scarcityBand', v_availability.scarcity_band,
    'availabilityVersion', v_availability.version,
    ...
    'failureBasisPoints', v_failure_basis_points,
    'failureRoll', v_failure_roll,
  ...
  insert into public.crafting_jobs (
    ...
    v_country,
    'standard',
    v_recipe.failure_rule,
    v_snapshot,
    v_now,
    v_now + make_interval(secs => ceil(
      v_recipe.base_duration_seconds
      * v_duration_multiplier
      * v_availability.event_duration_multiplier
      * v_availability.route_disruption_multiplier
    )::integer)
  ...
  loop
    ...
    if v_input.substitution_group is not null
      and p_substitutions ? v_input.substitution_group
    then
      v_item_key := lower(p_substitutions ->> v_input.substitution_group);
      select * into v_option
      from public.physical_economy_substitution_options
      where pack_id = v_pack_id
        and group_key = v_input.substitution_group
      ...
      if not found then
        raise exception 'CRAFTING_SUBSTITUTION_INVALID:%', v_input.substitution_group using errcode = 'P0001';
      end if;
    ...
    v_required := ceil(
      v_input.base_quantity
      * p_quantity
      * case when v_input.scaling_class = 'elastic_common' then v_input_multiplier else 1 end
      * v_ratio
    )::integer;
    ...
    for share;
    if not found then
      raise exception 'CRAFTING_INPUT_ITEM_UNAVAILABLE:%', v_item_key using errcode = 'P0001';
    end if;
    ...
    where h.game_session_id = p_game_session_id
      ...
      and h.game_item_id = v_game_item.id
    for update;
    if not found or v_holding.quantity_owned - v_holding.quantity_reserved < v_required then
      raise exception 'CRAFTING_INPUT_QUANTITY_UNAVAILABLE:%:%', v_item_key, v_required using errcode = 'P0001';
    end if;
    ...
    v_inventory_post := economy_private.post_inventory_transaction_v2(
      ...
      'job_started',
      v_job.id,
      p_idempotency_key || ':reserve:' || v_input.line_key,
      jsonb_build_object(
        'jobKey', v_job.public_id,
        'recipeKey', v_job.recipe_key,
        'itemKey', v_item_key,
    ...
    insert into public.inventory_reservations (
      ...
      v_required,
      'active',
      (v_inventory_post ->> 'transactionId')::uuid
    ...
    insert into public.crafting_job_inputs (
      ...
      v_input.base_quantity,
      v_required,
      v_input.substitution_group,
      v_ratio,
      v_penalty
    );
  end loop;

  insert into public.crafting_job_outputs (
    job_id,
    line_key,
    item_key,
    game_item_id,
  ...
  if (select count(*) from public.crafting_job_outputs o where o.job_id = v_job.id) <>
     (select count(*) from public.physical_economy_recipe_outputs o where o.recipe_id = v_recipe.id)
  then
    raise exception 'CRAFTING_OUTPUT_ITEM_UNAVAILABLE' using errcode = 'P0001';
  end if;
  ...
  set
    ...
    recipe_snapshot = recipe_snapshot || jsonb_build_object(
      'qualityPenaltyBasisPoints', v_quality_penalty,
...
revoke all on function public.start_player_crafting_job_v1(
  uuid, uuid, text, integer, jsonb, text
) from public, anon, authenticated;
grant execute on function public.start_player_crafting_job_v1(
  uuid, uuid, text, integer, jsonb, text
) to service_role;
