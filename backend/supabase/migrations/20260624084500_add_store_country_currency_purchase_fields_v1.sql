-- Add country-currency metadata to store quotes and purchases.
-- Existing currency_code/final_total_price remain backward-compatible player-debit fields.

ALTER TABLE public.store_purchase_quotes
ADD COLUMN IF NOT EXISTS item_currency_code text NULL,
ADD COLUMN IF NOT EXISTS player_currency_code text NULL,
ADD COLUMN IF NOT EXISTS exchange_rate numeric(18, 8) NULL,
ADD COLUMN IF NOT EXISTS item_local_final_unit_price numeric(12, 2) NULL,
ADD COLUMN IF NOT EXISTS item_local_final_total_price numeric(12, 2) NULL;
...
SET
  item_currency_code = coalesce(item_currency_code, currency_code),
  player_currency_code = coalesce(player_currency_code, currency_code),
  exchange_rate = coalesce(exchange_rate, 1),
  item_local_final_unit_price = coalesce(item_local_final_unit_price, final_unit_price),
  item_local_final_total_price = coalesce(item_local_final_total_price, final_total_price)
WHERE item_currency_code IS NULL
   OR player_currency_code IS NULL
   OR exchange_rate IS NULL
   OR item_local_final_unit_price IS NULL
   OR item_local_final_total_price IS NULL;
...
ALTER TABLE public.store_purchase_quotes
DROP CONSTRAINT IF EXISTS store_purchase_quotes_country_currency_metadata_check;

ALTER TABLE public.store_purchase_quotes
ADD CONSTRAINT store_purchase_quotes_country_currency_metadata_check
CHECK (
  (item_currency_code IS NULL OR (item_currency_code = upper(item_currency_code) AND length(item_currency_code) BETWEEN 3 AND 16))
  AND (player_currency_code IS NULL OR (player_currency_code = upper(player_currency_code) AND length(player_currency_code) BETWEEN 3 AND 16))
  AND (exchange_rate IS NULL OR exchange_rate > 0)
  AND (item_local_final_unit_price IS NULL OR item_local_final_unit_price >= 0)
  AND (item_local_final_total_price IS NULL OR item_local_final_total_price >= 0)
);

ALTER TABLE public.store_purchases
DROP CONSTRAINT IF EXISTS store_purchases_country_currency_metadata_check;

ALTER TABLE public.store_purchases
ADD CONSTRAINT store_purchases_country_currency_metadata_check
CHECK (
  (item_currency_code IS NULL OR (item_currency_code = upper(item_currency_code) AND length(item_currency_code) BETWEEN 3 AND 16))
  AND (player_currency_code IS NULL OR (player_currency_code = upper(player_currency_code) AND length(player_currency_code) BETWEEN 3 AND 16))
  AND (exchange_rate IS NULL OR exchange_rate > 0)
...
-- Fix ambiguous column references in store purchase execution RPC.
-- RETURNS TABLE output names are PL/pgSQL variables, so table columns with the same
-- names must be qualified inside SQL statements.
...
BEGIN
  ...
  SELECT mik.*
  INTO v_idempotency
  FROM public.mutation_idempotency_keys AS mik
  ...
  IF v_idempotency.status = 'COMPLETED' THEN
    ...
    WHERE ih.game_session_id = p_game_session_id
      AND ih.player_id = p_player_id
      AND ih.store_item_id = v_purchase.store_item_id;
  ...
  SELECT spq.*
  INTO v_quote
  FROM public.store_purchase_quotes AS spq
  WHERE spq.id = p_quote_id
    AND spq.game_session_id = p_game_session_id
    AND spq.player_id = p_player_id
  FOR UPDATE;
