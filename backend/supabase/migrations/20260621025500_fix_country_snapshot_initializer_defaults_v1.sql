-- Country snapshot initializer defaults patch V1.
-- Preserves default country-economy baseline values when a game has no explicit
-- game_country_economic_baseline_settings row.
...
comment on function public.initialize_country_economic_snapshots_for_game(uuid, timestamptz, text, jsonb) is
  'Creates initial real-time country_economic_snapshots for every active map country in one game session. Patched to use a named conflict constraint and preserve default baseline values when no explicit per-game baseline row exists.';
