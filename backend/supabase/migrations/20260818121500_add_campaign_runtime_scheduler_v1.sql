...
revoke all on function public.configure_campaign_runtime_scheduler_v1(text)
  from public, anon, authenticated;
grant execute on function public.configure_campaign_runtime_scheduler_v1(text)
  to service_role;

comment on function public.configure_campaign_runtime_scheduler_v1(text) is
  'Creates or rotates a dedicated Vault-backed campaign scheduler token, records only its SHA-256 verifier, replaces the named Cron job through cron.unschedule/cron.schedule, and invokes the campaign-orchestrator every minute.';
