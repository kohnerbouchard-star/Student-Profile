-- Align Arrival package funding with the canonical Player checking account.
-- Arrival grants previously credited the retired cash account, leaving the
-- travel settlement account unfunded after a successful questionnaire.
...
-- The original residency RPC used an unqualified `revision` expression inside
-- a RETURNS TABLE function. PostgreSQL treated it as ambiguous between the
-- output column and the table column. Keep the existing authorization,
-- eligibility, replay, and revision rules while qualifying the mutation.
...
begin
  ...
  update public.player_residency_states as state_row
  set pending_country_id = p_target_country_id,
      revision = state_row.revision + 1,
      updated_at = greatest(p_requested_at, state_row.updated_at)
...
comment on function public.apply_arrival_grant_command_v1(
  ...
  'Processes one server-created Arrival grant command exactly once and credits the canonical Player checking account.';

comment on function public.request_player_residency_change_v1(
  uuid, uuid, text, bigint, timestamptz
) is
  'Requests a replay-safe residency change with qualified revision mutation and monotonic timestamps.';
