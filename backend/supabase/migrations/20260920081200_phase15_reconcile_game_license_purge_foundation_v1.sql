...
-- The current repository migrations immediately preceding this forward
-- reconciliation already rebuild the complete purge registry and deterministic
-- delete order from the final schema. Preserve that current 207/456/206
-- fingerprint here; rebuilding with the historical public-only algorithm would
-- regress the fail-closed purge contract.
