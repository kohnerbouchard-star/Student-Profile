...
begin
  if p_batch_limit is null or p_batch_limit not between 1 and 10000 then
    raise exception using errcode = '22023', message = 'rate limit cleanup batch must be between 1 and 10000';
  end if;
  ...
  return query
  select v_deleted_count, count(*)::bigint, min(buckets.expires_at)
  from public.request_rate_limit_buckets as buckets
  where buckets.expires_at <= v_now;
end;
$$;
...
begin
  if p_since_seconds is null or p_since_seconds not between 60 and 86400 then
    raise exception using errcode = '22023', message = 'rate limit telemetry window must be between 60 and 86400 seconds';
  end if;
  if p_row_limit is null or p_row_limit not between 1 and 100 then
    raise exception using errcode = '22023', message = 'rate limit telemetry row limit must be between 1 and 100';
  end if;
  ...
  select
    ...
    buckets.limit_count,
    count(*)::bigint,
    count(*) filter (where buckets.blocked_until > clock_timestamp())::bigint,
    coalesce(sum(buckets.request_count), 0)::bigint,
    min(buckets.last_request_at),
    max(buckets.last_request_at),
    min(buckets.expires_at)
  from public.request_rate_limit_buckets as buckets
  where buckets.last_request_at >= clock_timestamp() - make_interval(secs => p_since_seconds)
  group by buckets.dimension, buckets.window_seconds, buckets.limit_count
  order by 5 desc, 6 desc, buckets.dimension, buckets.window_seconds, buckets.limit_count
  limit p_row_limit;
end;
$$;
