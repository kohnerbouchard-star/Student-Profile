...
-- Forward-only repair for the demo Story initializer. The original activation
-- UPSERT used an unqualified conflict target that collided with RETURNS TABLE
-- variables inside PL/pgSQL. Preserve all Story content and behavior while
-- binding the conflict target to the named table constraint.
...
begin
  ...
  where event_row.storyline_id = v_storyline_id
  ...
  from public.game_session_storylines as activation_row
  where activation_row.game_session_id = p_game_session_id
    and activation_row.storyline_id = v_storyline_id
...
comment on function public.initialize_demo_storyline_for_game(uuid, text) is
  'Creates the deterministic Econovaria demo storyline content if missing and activates it for one game session. V2 qualifies the activation conflict target to avoid PL/pgSQL output-variable ambiguity.';
