-- Stock market seed/copy RPC ambiguity fix V1.
-- Replaces initialize_stock_market_assets_for_game so PL/pgSQL output columns do
-- not conflict with inserted table column references in INSERT ... RETURNING.
...
begin
  ...
  with templates_to_copy as (
    ...
    insert into public.game_session_stock_assets as inserted_asset (
      game_session_id,
      template_id,
      ticker,
      company_name,
    ...
    returning
      inserted_asset.id as stock_asset_id,
      inserted_asset.game_session_id as inserted_game_session_id,
      inserted_asset.ticker,
      inserted_asset.current_price,
      inserted_asset.long_run_volatility
  ), inserted_ticks as (
    ...
    select
      inserted_assets.inserted_game_session_id,
      inserted_assets.stock_asset_id,
      0,
      ...
      jsonb_build_object(
        ...
        'gameSessionId', inserted_assets.inserted_game_session_id,
...
comment on function public.initialize_stock_market_assets_for_game(uuid, text) is
  'Copies active stock templates into one game session and inserts tick_index 0 baseline stock_price_ticks for newly inserted assets. Intended for trusted service-role backend use only. Column references are aliased to avoid PL/pgSQL output-column ambiguity.';
