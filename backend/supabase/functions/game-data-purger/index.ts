...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    if (stage === "r2") {
      return json(200, await purgeR2Stage(client, supabaseUrl, requestId, gameSessionId));
    }
    if (stage === "db") {
...
function assertPreflight(preflight: Record<string, unknown>, gameSessionId: string) {
  ...
  if (String(preflight.deleteOrderSha256 || "") !== EXPECTED_DELETE_ORDER_SHA256) {
    throw new Error("preflight_delete_order_digest_mismatch");
  }
  if (Number(preflight.deleteOrderTableCount || 0) !== EXPECTED_DELETE_ORDER_TABLES) {
...
async function purgeR2Stage(
  ...
  if (objects.length > 0) {
    ...
    if ((deleted.Errors || []).length > 0) {
      throw new Error(`r2_delete_objects_failed:${JSON.stringify(deleted.Errors).slice(0, 800)}`);
    }
