...
const THREAD_ACTION = `staff.messages.thr_${"a".repeat(24)}`;
...
Deno.test("Admin Messaging retains HTML-shaped text only as inert bounded content", async () => {
  const service = new FakeService([{
  ...
  assertEquals(service.calls[0].args.p_initial_body, "<script>alert('text only')</script>");
});
...
Deno.test("expired-thread deletion is typed replay-safe and UUID-private", async () => {
  ...
  const result = await operation(service, {
    ...
    body: { reason: "Retention expired.", idempotencyKey: "retention:delete:1" },
  });
  assertEquals(result.status, 200);
  assertEquals(service.calls[0].name, "delete_expired_admin_message_thread_atomic_v1");
  assertNoUuid(JSON.stringify(result.body));
});

Deno.test("staff Messaging limiter separates policy/create and unifies commands per thread", async () => {
  for (const [method, suffix, query, action] of [
    ["GET", "/messages", "", "staff.messages.read"],
    ["GET", "/messages", "q=market", "staff.messages.search"],
    ["GET", "/messages/policy", "", "staff.messages.policy.read"],
    ["POST", "/messages/policy", "", "staff.messages.policy.write"],
    ["POST", "/messages/threads", "", "staff.messages.create"],
    ["POST", `/messages/threads/${THREAD}/disable`, "", THREAD_ACTION],
    ["POST", `/messages/threads/${THREAD}/messages/${MESSAGE}/hide`, "", THREAD_ACTION],
    ["POST", `/messages/threads/${THREAD}/delete`, "", THREAD_ACTION],
  ] as const) {
    const request = new Request(`https://example.test/games/${GAME}${suffix}${query ? `?${query}` : ""}`, { method });
    assertEquals(readStaffMessagingRateLimitOperation({ request, suffix })?.action, action);
  }
...
function operation(service: FakeService, options: { readonly method?: string; readonly suffix?: string; readonly query?: string; readonly body?: unknown }) {
