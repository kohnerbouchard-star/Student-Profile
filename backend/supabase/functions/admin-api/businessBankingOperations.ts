...
export async function handleBusinessBankingAdminOperation(
  ...
  try {
    ...
    if (input.suffix === "/loan-products" && input.request.method === "POST") {
    ...
    const loanRecovery = input.suffix.match(/^\/loans\/(lon_[0-9a-f]{32})\/restructure$/u);
...
function publicKey(value: unknown, prefix: string): string {
  const result = typeof value === "string" ? value.trim().toLowerCase() : "";
  if (!new RegExp(`^${prefix}_[0-9a-f]{32}$`, "u").test(result)) throw new Error("INVALID_PUBLIC_KEY");
  return result;
}
