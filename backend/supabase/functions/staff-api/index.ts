...
import {
  readGameSettingsRoutePath,
} from "../../../src/domains/game-sessions/api/gameSettingsRoutePaths.ts";
import {
...
Deno.serve(async (request: Request) => {
  ...
  if (!env.ok) {
    return jsonError(500, {
      code: "staff_runtime_not_configured",
      message: "The staff API runtime is not configured.",
      retryable: false,
    });
  }
  ...
  if (gameSettingsRoute) {
    return handleGameSettingsRequest(
      request,
      gameSettingsRoute.gameSessionId,
      { resolveStaffForRequest },
    );
  }
  ...
  if (staffStoreCatalogRoute) {
    return handleStaffStoreCatalogRequest(
      request,
      staffStoreCatalogRoute,
      { resolveStaffForRequest },
    );
  }
  ...
  if (staffContractRoute) {
    return handleStaffContractRequest(
      request,
      staffContractRoute,
      { resolveStaffForRequest },
    );
  ...
  if (playerRosterRoute?.kind === "players") {
    return handlePlayerRosterRequest(
      request,
  ...
  return jsonError(404, {
    ...
    message: "Staff API route was not found.",
    retryable: false,
  });
});
