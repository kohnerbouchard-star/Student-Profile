...
interface StoreItemsBody {
  ...
  readonly generatedAt: string;
  readonly items: readonly StoreItemView[];
}

interface PlayerStoreItemsBody {
  readonly ok: true;
  readonly gameSession: {
    readonly id: string;
    readonly name: string;
...
interface CreateStoreItemRequestBody {
...
interface UpdateStoreItemRequestBody {
  readonly name?: string;
  readonly description?: string | null;
  readonly category?: string;
  readonly price?: number;
...
interface StaffStoreItemsRoute {
  readonly gameSessionId: string;
...
Deno.serve(async (request) => {
  ...
  if (url.pathname.endsWith("/players/me/store/items")) {
    return handlePlayerStoreItemsRequest(request);
  }
  ...
  const staffStoreItemsRoute = readStaffStoreItemsRoutePath(url.pathname);

  if (staffStoreItemsRoute) {
    return handleStaffStoreItemsRequest(
      request,
      staffStoreItemsRoute.gameSessionId,
      staffStoreItemsRoute.itemId,
    );
  }
...
async function handleStaffStoreItemsRequest(
  request: Request,
  gameSessionId: string,
  itemId: string | null,
): Promise<Response> {
  if (itemId === null && request.method !== "GET" && request.method !== "POST") {
    return jsonError(405, {
      code: "method_not_allowed",
      message: "Use GET or POST for store items.",
      retryable: false,
    });
  }

  if (itemId !== null && request.method !== "PATCH") {
    return jsonError(405, {
      code: "method_not_allowed",
      message: "Use PATCH to update a store item.",
      retryable: false,
    });
  }
  ...
  try {
    ...
    if (request.method === "POST") {
      const body = await readCreateStoreItemRequestBody(request);
      const itemKey = body.itemKey ?? buildStoreItemKeyFromName(body.name);
      ...
      const createResponse = await staffResult.serviceClient
        ...
        .select("id,item_key,name,description,category,price,currency_code,stock_quantity,status,visibility,sort_order,created_at,updated_at")
        .single();

      if (createResponse.error || !createResponse.data?.id) {
        return jsonError(500, {
          code: "store_item_create_failed",
          message: "Store item could not be created.",
          retryable: false,
        });
      }

      return jsonResponse<StoreItemsBody>(201, {
        ok: true,
        gameSession: {
        ...
        items: [mapStoreItemRow(createResponse.data as StoreItemRow)],
      });
    }

    if (request.method === "PATCH" && itemId !== null) {
      const body = await readUpdateStoreItemRequestBody(request);
      const updatePayload = buildStoreItemUpdatePayload(body);

      if (Object.keys(updatePayload).length === 0) {
        return jsonError(400, {
          code: "store_item_update_empty",
          message: "At least one store item field must be provided.",
          retryable: false,
        });
      }

      const updateResponse = await staffResult.serviceClient
        .from("store_items")
      ...
      if (!updateResponse.data?.id) {
        return jsonError(404, {
          ...
          message: "Store item was not found for this game.",
          retryable: false,
        });
      }

      return jsonResponse<StoreItemsBody>(200, {
        ok: true,
        gameSession: {
          id: ownershipResult.gameSession.id,
          name: ownershipResult.gameSession.name,
        ...
        generatedAt: new Date().toISOString(),
        items: [mapStoreItemRow(updateResponse.data as StoreItemRow)],
      });
    }

    const itemsResponse = await staffResult.serviceClient
      .from("store_items")
      .select("id,item_key,name,description,category,price,currency_code,stock_quantity,status,visibility,sort_order,created_at,updated_at")
      .eq("game_session_id", gameSessionId)
    ...
    return jsonResponse<StoreItemsBody>(200, {
      ...
      generatedAt: new Date().toISOString(),
      items: ((itemsResponse.data ?? []) as StoreItemRow[]).map(mapStoreItemRow),
    });
  } catch (error) {
    ...
    return jsonError(500, {
      code: "store_items_failed",
      message: "Store items request failed.",
      retryable: false,
    });
  }
}

async function handlePlayerStoreItemsRequest(
  request: Request,
): Promise<Response> {
  ...
  try {
    ...
    const itemsResponse = await serviceClient
      .from("store_items")
      .select("id,item_key,name,description,category,price,currency_code,stock_quantity,status,visibility,sort_order,created_at,updated_at")
    ...
    if (itemsResponse.error) {
      return jsonError(500, {
        code: "player_store_items_failed",
        message: "Store items could not be loaded.",
        retryable: false,
      });
    }

    return jsonResponse<PlayerStoreItemsBody>(200, {
      ok: true,
      gameSession: {
...
async function handleLicensingActivationRequest(
  ...
  try {
    ...
    const storeSeedResponse = await serviceClient.rpc(
      "seed_default_store_items",
      {
        p_game_session_id: activationRow.game_session_id,
      },
    );

    if (storeSeedResponse.error) {
      return jsonError(500, {
        code: "store_seed_failed",
        message: "Default store items could not be created.",
        retryable: false,
      });
    }
...
function readStaffStoreItemsRoutePath(
  pathname: string,
): StaffStoreItemsRoute | null {
  ...
  const storeSegment = segments[gamesIndex + 2];
  const itemsSegment = segments[gamesIndex + 3];
  const itemId = segments[gamesIndex + 4];
  ...
  if (
    ...
    return { gameSessionId, itemId: null };
  }
  ...
  if (
    ...
    return { gameSessionId, itemId };
  }

  return null;
}

async function readCreateStoreItemRequestBody(
  request: Request,
): Promise<CreateStoreItemRequestBody> {
  const value = await readJsonObjectRequestBody(request);

  return {
    itemKey: parseOptionalStoreItemKey(value.itemKey),
    name: parseRequiredText(
      value.name,
      "store_item_name_required",
      "name is required.",
    ),
    description: parseOptionalText(value.description),
    category: normalizeStoreCategory(parseOptionalText(value.category) ?? "general"),
    price: parseStoreMoneyAmount(value.price, 0, "price"),
    currencyCode: normalizeCurrencyCode(parseOptionalText(value.currencyCode) ?? "ECO"),
    stockQuantity: parseStoreQuantity(value.stockQuantity, 0, "stockQuantity"),
    status: normalizeStoreItemStatus(parseOptionalText(value.status) ?? "active"),
    visibility: normalizeStoreItemVisibility(parseOptionalText(value.visibility) ?? "visible"),
    sortOrder: parseStoreSortOrder(value.sortOrder, 0),
  };
}

async function readUpdateStoreItemRequestBody(
  request: Request,
): Promise<UpdateStoreItemRequestBody> {
  const value = await readJsonObjectRequestBody(request);
  const body: Record<string, unknown> = {};

  if ("name" in value) {
    body.name = parseRequiredText(
      value.name,
      "store_item_name_required",
      "name is required.",
    );
  ...
  if ("description" in value) {
    body.description = parseNullableText(value.description);
  }

  if ("category" in value) {
    body.category = normalizeStoreCategory(
      parseRequiredText(
        value.category,
        "store_item_category_required",
  ...
  if ("price" in value) {
    body.price = parseStoreMoneyAmount(value.price, 0, "price");
  }

  if ("currencyCode" in value) {
    body.currencyCode = normalizeCurrencyCode(
      parseRequiredText(
        value.currencyCode,
        "store_item_currency_required",
  ...
  if ("stockQuantity" in value) {
    body.stockQuantity = parseStoreQuantity(value.stockQuantity, 0, "stockQuantity");
  }

  if ("status" in value) {
    body.status = normalizeStoreItemStatus(
      parseRequiredText(
        value.status,
        "store_item_status_required",
        "status is required.",
      ),
    );
  }
  ...
  if ("visibility" in value) {
    body.visibility = normalizeStoreItemVisibility(
      parseRequiredText(
        value.visibility,
        "store_item_visibility_required",
  ...
  if ("sortOrder" in value) {
    body.sortOrder = parseStoreSortOrder(value.sortOrder, 0);
  }

  return body as UpdateStoreItemRequestBody;
}
...
function buildStoreItemUpdatePayload(
  body: UpdateStoreItemRequestBody,
  ...
  if (body.currencyCode !== undefined) {
    payload.currency_code = body.currencyCode;
  }

  if (body.stockQuantity !== undefined) {
    payload.stock_quantity = body.stockQuantity;
  }
...
function parseOptionalStoreItemKey(value: unknown): string | null {
...
function buildStoreItemKeyFromName(name: string): string {
  const slug = name
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "_")
  ...
  if (!slug) {
    throw new EdgeActivationError(
      "invalid_store_item_key",
      "A store item key could not be generated from the name.",
      400,
    );
  }
...
function normalizeStoreCategory(value: string): string {
  const normalizedValue = value.trim().toLowerCase();

  if (!/^[a-z0-9_-]{1,32}$/.test(normalizedValue)) {
    throw new EdgeActivationError(
      "invalid_store_category",
      "category must be 1 to 32 letters, numbers, underscores, or hyphens.",
      400,
    );
  }
...
function parseStoreMoneyAmount(
  value: unknown,
  defaultValue: number,
  fieldName: string,
): number {
...
function parseStoreSortOrder(value: unknown, defaultValue: number): number {
  if (value === undefined || value === null || value === "") {
    return defaultValue;
  }
...
function normalizeStoreItemStatus(value: string): string {
  const normalizedValue = value.trim().toLowerCase();

  if (!["active", "disabled", "archived"].includes(normalizedValue)) {
    throw new EdgeActivationError(
      "invalid_store_item_status",
...
function normalizeStoreItemVisibility(value: string): string {
  const normalizedValue = value.trim().toLowerCase();

  if (!["visible", "hidden"].includes(normalizedValue)) {
    throw new EdgeActivationError(
      "invalid_store_item_visibility",
...
function mapStoreItemRow(row: StoreItemRow): StoreItemView {
  return {
    id: row.id,
    ...
    price: readBalanceNumber(row.price),
