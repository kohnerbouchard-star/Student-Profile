...
function renderConfirmation(request: Request): Response {
  ...
  if (type !== "email" || !TOKEN_HASH_PATTERN.test(tokenHash)) {
    return invalidRequest();
  }
...
async function consumeConfirmation(request: Request): Promise<Response> {
  ...
  if (String(request.headers.get("origin") || "") !== runtime.supabaseUrl) {
    return verificationFailure(403);
  }
  ...
  // Confirmation can mint an AAL1 session. It is immediately revoked because
  // the administrator must still enter the password and complete TOTP through
  // the application-owned HttpOnly web-session boundary.
...
function readRuntimeConfig(): RuntimeConfig | null {
  ...
  const returnUrl = String(
    Deno.env.get("ECONOVARIA_EMAIL_VERIFICATION_RETURN_URL") || DEFAULT_RETURN_URL,
  ).trim();
  try {
...
function verificationFailure(status = 400): Response {
...
<body><main class="shell"><section class="card" aria-labelledby="error-title"><span class="kicker">Econovaria account security</span><h1 id="error-title">${escapeHtml(title)}</h1><p>${escapeHtml(message)}</p><a class="back" href="${DEFAULT_RETURN_URL}" rel="noreferrer">Return to administrator sign-in</a></section></main></body></html>`;
}
