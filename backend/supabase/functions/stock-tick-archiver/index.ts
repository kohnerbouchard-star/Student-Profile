...
Deno.serve(async (request: Request) => {
  ...
  if (!supabaseUrl || !serviceRoleKey || !r2Endpoint || !r2AccessKeyId || !r2SecretAccessKey || !r2Bucket) {
    return json(500, {
      ok: false,
      error: { code: "archive_runtime_config_missing", message: "Required archive runtime configuration is missing." },
    });
  }
  ...
  if (action !== "archive_next_hour" && action !== "archive_batch") {
    return json(400, {
      ok: false,
      error: { code: "unsupported_action", message: "Supported actions: archive_next_hour, archive_batch." },
    });
  }
  ...
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu.test(gameSessionId)) {
    return json(400, {
      ok: false,
      error: { code: "invalid_game_session_id", message: "A valid gameSessionId is required." },
    });
  }
  ...
  const maxHours = Math.max(
    1,
    Math.min(MAX_BATCH_HOURS, Number.isFinite(requestedHours) ? Math.trunc(requestedHours) : 1),
  );
  ...
  try {
    ...
    return json(500, {
      ...
      error: {
        code: "stock_tick_archive_failed",
        message: error instanceof Error ? error.message : "Stock tick archive failed.",
      },
    });
  }
});
...
async function archiveOne(
  ...
  const preparedResponse = await client.rpc("prepare_next_stock_tick_archive", {
  ...
  if (!Number.isSafeInteger(expectedRows) || expectedRows <= 0 || expectedRows > 250000) {
    throw new Error("prepared_row_count_invalid");
  }
  const rows = await readPreparedRows(client, prepared, expectedRows);
  if (rows.length !== expectedRows) {
    throw new Error(`archive_row_count_mismatch:expected=${expectedRows}:actual=${rows.length}`);
  }

  const parquetBuffer = parquetWriteBuffer({
    columnData: [
      { name: "id", data: rows.map((row) => row.id), type: "STRING" },
      { name: "game_session_id", data: rows.map((row) => row.game_session_id), type: "STRING" },
      { name: "stock_asset_id", data: rows.map((row) => row.stock_asset_id), type: "STRING" },
      { name: "tick_index", data: rows.map((row) => Number(row.tick_index)), type: "INT32" },
      { name: "ticker", data: rows.map((row) => row.ticker), type: "STRING" },
      { name: "price", data: rows.map((row) => String(row.price)), type: "STRING" },
      { name: "previous_price", data: rows.map((row) => String(row.previous_price)), type: "STRING" },
      { name: "log_return", data: rows.map((row) => String(row.log_return)), type: "STRING" },
      { name: "change_pct", data: rows.map((row) => String(row.change_pct)), type: "STRING" },
      { name: "volume", data: rows.map((row) => String(row.volume)), type: "STRING" },
      { name: "current_volatility", data: rows.map((row) => String(row.current_volatility)), type: "STRING" },
      { name: "long_run_volatility", data: rows.map((row) => String(row.long_run_volatility)), type: "STRING" },
      { name: "explanation", data: rows.map((row) => row.explanation), type: "JSON" },
      { name: "created_at", data: rows.map((row) => row.created_at), type: "STRING" },
    ],
    codec: "SNAPPY",
    rowGroupSize: 10000,
    kvMetadata: [
  ...
  if (head.status === 404) {
    ...
    if (!put.ok && put.status !== 412) {
      throw new Error(`r2_put_failed:${put.status}:${await safeText(put)}`);
    }
  ...
  if (remoteLength !== compressedBytes) {
    throw new Error(`r2_length_mismatch:expected=${compressedBytes}:actual=${remoteLength}`);
  }

  const verifyGet = await r2.fetch(objectUrl, { method: "GET" });
  if (!verifyGet.ok) {
    throw new Error(`r2_get_verify_failed:${verifyGet.status}:${await safeText(verifyGet)}`);
  }
  const remoteBuffer = await verifyGet.arrayBuffer();
  if (remoteBuffer.byteLength !== compressedBytes) {
    throw new Error(`r2_verify_length_mismatch:expected=${compressedBytes}:actual=${remoteBuffer.byteLength}`);
  }
  ...
  let purgedRows = 0;
  if (purge) {
    const purgeResponse = await client.rpc("purge_verified_stock_tick_archive", {
    ...
    if (purgedRows !== expectedRows) {
      throw new Error(`purge_count_mismatch:expected=${expectedRows}:actual=${purgedRows}`);
    }
  }
...
async function readPreparedRows(
  ...
  for (let offset = 0; offset < expectedRows; offset += PAGE_SIZE) {
    ...
    const response = await client.from("stock_price_ticks").select(TICK_SELECT)
      ...
      .gte("tick_index", Number(prepared.min_tick_index))
      .lte("tick_index", Number(prepared.max_tick_index))
      .gte("created_at", prepared.range_start)
      .lt("created_at", prepared.range_end)
      .order("tick_index", { ascending: true })
      .order("stock_asset_id", { ascending: true })
      .range(offset, end);
  ...
  rows.sort((left, right) =>
    Number(left.tick_index) - Number(right.tick_index) || left.stock_asset_id.localeCompare(right.stock_asset_id)
  );
  return rows;
}
...
function archiveObjectKey(supabaseUrl: string, prepared: PreparedArchive): string {
  ...
  const environment = ref === "eecvbssdvarfcykcfrny"
    ? "staging"
    : ref === "cgiukdjwicykrmtkhudh"
    ? "production"
    : ref;
