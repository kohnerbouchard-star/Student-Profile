...
async function readPreparedRows(client: SupabaseClient, prepared: PreparedArchive, expectedRows: number): Promise<TickRow[]> {
  const minTick = Number(prepared.min_tick_index);
  const maxTick = Number(prepared.max_tick_index);
  const tickCount = maxTick - minTick + 1;
  if (!Number.isSafeInteger(minTick) || !Number.isSafeInteger(maxTick) || tickCount <= 0) {
    throw new Error("prepared_tick_range_invalid");
  }

  const rowsPerTick = Math.max(1, Math.ceil(expectedRows / tickCount));
  const ticksPerBatch = Math.max(1, Math.floor((PAGE_SIZE - 1) / rowsPerTick));
  const rows: TickRow[] = [];

  for (let tickStart = minTick; tickStart <= maxTick; tickStart += ticksPerBatch) {
    const tickEnd = Math.min(maxTick, tickStart + ticksPerBatch - 1);
    const response = await client.from("stock_price_ticks").select(TICK_SELECT)
      .eq("game_session_id", prepared.game_session_id)
      .gte("tick_index", tickStart).lte("tick_index", tickEnd)
