...
Deno.test("trusted Progression contract accepts bounded World and Messaging events", async () => {
  for (
    const [sourceDomain, eventType, sourcePublicId] of [
    ...
    assertEquals(
      (client.calls[0]?.args as { p_source_domain?: string }).p_source_domain,
      sourceDomain,
    );
    assertEquals(
      (client.calls[0]?.args as { p_event_type?: string }).p_event_type,
      eventType,
    );
  }
});
...
Deno.test("trusted Progression event rejects source and event mismatches before RPC", async () => {
  ...
  const errors = await capture(() =>
    recordTrustedProgressionEventV1(client as never, {
      gameId: GAME,
      playerUuid: PLAYER,
...
Deno.test("source-event mutation conflicts fail closed with a stable public error", async () => {
  const client = new FakeClient([], {
    message: "PROGRESSION_SOURCE_EVENT_CONFLICT",
  });
