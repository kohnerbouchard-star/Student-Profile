...
export const PLAYER_ROUTE_CAPABILITY_KEYS = [
  ...
  "marketplace", "inventory", "crafting", "banking", "loans", "messages",
  "progression", "profile",
] as const;
...
export const PLAYER_ACTION_CAPABILITY_KEYS = [
  ...
  "marketOrder", "marketSearch", "marketWatchlist", "marketplaceActivate",
  "marketplaceCancel", "marketplaceListing", "marketplacePurchase",
  "marketplaceDispute", "messageAttachment", "messageSearch", "messageSend",
  "notificationsRead", "progressionClaim", "progressionUnlock", "savingsTransfer",
  "storePurchase", "storyDeliveryState",
] as const;
...
export interface PlayerCapabilityEndpointOperation { readonly method: PlayerCapabilityHttpMethod; readonly pathTemplate: string }
...
const REVIEWED_ENDPOINTS: readonly PlayerCapabilityEndpointDescriptor[] = [
  ...
  { key: "marketplace", operations: [{ method: "GET", pathTemplate: "/players/me/marketplace/listings" }], routeCapabilities: ["marketplace"] },
  { key: "marketplaceListing", operations: [{ method: "POST", pathTemplate: "/players/me/marketplace/listings" }], actionCapabilities: ["marketplaceListing"] },
  { key: "marketplaceActivate", operations: [{ method: "POST", pathTemplate: "/players/me/marketplace/listings/:listingId/activate" }], actionCapabilities: ["marketplaceActivate"] },
  { key: "marketplacePurchase", operations: [{ method: "POST", pathTemplate: "/players/me/marketplace/listings/:listingId/purchase" }], actionCapabilities: ["marketplacePurchase"] },
  { key: "marketplaceCancel", operations: [{ method: "POST", pathTemplate: "/players/me/marketplace/listings/:listingId/cancel" }], actionCapabilities: ["marketplaceCancel"] },
  { key: "marketplaceDispute", operations: [{ method: "POST", pathTemplate: "/players/me/marketplace/orders/:orderId/disputes" }], actionCapabilities: ["marketplaceDispute"] },
...
export function buildPlayerCapabilityManifest(): PlayerCapabilityManifest {
  const routeCapabilities = new Set(REVIEWED_ENDPOINTS.flatMap((descriptor) => descriptor.routeCapabilities ?? []));
  const actionCapabilities = new Set(REVIEWED_ENDPOINTS.flatMap((descriptor) => descriptor.actionCapabilities ?? []));
  return Object.freeze({
    ...
    endpoints: Object.freeze(REVIEWED_ENDPOINTS.map((descriptor) => Object.freeze({
      key: descriptor.key,
      operations: Object.freeze(descriptor.operations.map((operation) => Object.freeze({ ...operation }))),
    }))),
  });
}
function capabilityFlags<TKey extends string>(keys: readonly TKey[], supported: ReadonlySet<TKey>): Readonly<Record<TKey, boolean>> {
