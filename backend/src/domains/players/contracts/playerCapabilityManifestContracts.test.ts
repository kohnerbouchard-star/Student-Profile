...
Deno.test("player capability manifest publishes only reviewed operations", () => {
  ...
  for (const key of [
    ...
    "inventory", "store", "banking", "progression",
  ...
  for (const key of [
    ...
    "storyDeliveryState", "progressionUnlock", "progressionClaim",
  ...
  for (const key of [
    ...
    "progression", "progressionUnlock", "progressionClaim",
...
Deno.test("every advertised endpoint path is recognized by authoritative dispatch", () => {
  ...
  for (const operation of operations) {
    const parsed = operation.key === "bootstrap"
      ...
      : operation.key === "capabilities" ? readPlayerCapabilityManifestRoutePath(operation.path)
      : operation.key === "banking" ? readPlayerBankingPublicRoutePath(operation.path)
      : operation.key === "contractAccept" ? readPlayerContractAcceptanceRoutePath(operation.path)
      : operation.key === "contractSubmit" ? readPlayerContractPublicSubmitRoutePath(operation.path)
      : operation.key === "contracts" ? readPlayerContractPublicListRoutePath(operation.path)
      ...
      : operation.key === "inventory" ? readPlayerInventoryRoutePath(operation.path)
      : operation.key === "inventoryRedemptions" ? readPlayerInventoryRedemptionRoutePath(operation.path)
      : operation.key === "progression" || operation.key === "progressionUnlock" || operation.key === "progressionClaim"
      ...
      : operation.key === "logout" ? readPlayerSessionLogoutRoutePath(operation.path)
      : null;
