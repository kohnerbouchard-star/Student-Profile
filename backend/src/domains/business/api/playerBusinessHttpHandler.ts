...
function validateEnvelope(request: Request): void {
  ...
  if ([...url.searchParams.keys()].length) throw invalidRequest("Business routes do not accept query parameters.");
...
function readFormationOwners(value: unknown): readonly Record<string, unknown>[] {
  if (!Array.isArray(value) || value.length < 1 || value.length > 16) throw invalidRequest("owners must contain 1-16 proposed owners.");
...
function invalidRequest(message: string, status = 400, code = "invalid_business_request"): PlayerBusinessError {
