...
Deno.test("Inventory redemption routes recognize direct and Edge public-ID paths", () => {
  ...
  assertEquals(
    readPlayerInventoryRedemptionRoutePath(
      "/functions/v1/player-api/players/me/inventory/meal-pass/redemptions",
    ),
    {
      kind: "request",
      itemId: "meal-pass",
  ...
  assertEquals(
    readPlayerInventoryRedemptionRoutePath(
      "/player-api/players/me/inventory/redemptions",
    ),
    {
      kind: "collection",
    },
  ...
  assertEquals(
    readPlayerInventoryRedemptionRoutePath(
      `/functions/v1/player-api/players/me/inventory/redemptions/${REQUEST_ID}`,
    ),
    {
      kind: "item",
      requestId: REQUEST_ID,
...
Deno.test("Inventory redemption routes reject UUID-like and malformed browser identifiers", () => {
  for (
    const path of [
      ...
      "/functions/v1/player-api/players/me/inventory/00000000-0000-4000-8000-000000000001/redemptions",
    ]
  ) {
