...
export function evaluateStoryCondition(
  ...
  if ("all" in condition) {
    return condition.all.every((child) => evaluateStoryCondition(child, player));
  }
  if ("any" in condition) {
  ...
  switch (condition.type) {
    ...
    case "player_relationship_story_decision_in": {
      ...
      const optionKey = readText(decisions?.[condition.decisionKey]);
...
function readStoryFlag(flags: unknown, flagKey: string, expected: JsonValue): boolean {
  if (!isRecord(flags) || !Object.prototype.hasOwnProperty.call(flags, flagKey)) {
    return false;
  }
