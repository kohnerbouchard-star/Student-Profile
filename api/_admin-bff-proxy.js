...
async function runtimeHealthRoute(request, response, options = {}) {
  ...
  try {
    const result = await checkRuntimeHealth(options);
...
function readHealthConfiguration(environment = process.env) {
  ...
  if (!new Set(["staging", "production"]).has(runtimeEnvironment)) {
...
async function probeHealthService({ service, supabaseUrl, fetchImpl, timeoutMs }) {
  ...
  try {
    ...
    const payload = await readBoundedHealthJson(upstream);
    return Object.freeze({
      service,
...
async function readBoundedHealthJson(response) {
  const text = await response.text();
  if (Buffer.byteLength(text, "utf8") > MAX_HEALTH_RESPONSE_BYTES) return null;
  try {
...
function readConfig(environment = process.env) {
  const supabaseUrl = String(environment.ECONOVARIA_SUPABASE_URL || "")
    .trim()
    .replace(/\/+$/, "");
  const publishableKey = String(
    environment.ECONOVARIA_SUPABASE_PUBLISHABLE_KEY || ""
  ).trim();
  let parsed;
  try {
