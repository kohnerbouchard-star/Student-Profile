...
// Supabase publishable keys are intentionally public client credentials. Keeping
// the staging key here lets the canonical Econovaria web origin complete
// staging acceptance without exposing any service-role or management secret.
  ...
  try {
    ...
    if (String(request.method || "GET").toUpperCase() !== "POST") {
      return sendJson(response, 405, errorBody(
        "method_not_allowed",
        "Use POST to continue the authentication review."
      ));
    }
    ...
    if (!bodyResult.ok) {
      return sendJson(response, bodyResult.status, errorBody(
        bodyResult.code,
        bodyResult.message
      ));
    }
    ...
    const projectRef = String(bodyResult.value.projectRef || "").trim().toLowerCase();
    ...
    const config = projectConfig(projectRef);
    ...
    const logout = await fetch(`${config.supabaseUrl}/auth/v1/logout?scope=local`, {
      method: "POST",
      headers: {
        apikey: config.publishableKey,
...
function projectConfig(projectRef) {
  ...
  if (projectRef !== PRODUCTION_PROJECT_REF) {
    throw new Error("unsupported auth project");
  }
...
function readJsonBody(request) {
  ...
  if (Number.isFinite(declared) && declared > MAX_BODY_BYTES) {
    return {
      ...
      message: "Authentication review request is too large."
    };
  }
  ...
  if (request.body === undefined || request.body === null) {
    ...
    bytes = Buffer.from(request.body, "utf8");
  } else {
  ...
  if (bytes.byteLength === 0) {
    return {
      ...
      message: "A JSON authentication review request is required."
    };
  }
  ...
  try {
    ...
    return {
      ...
      message: "Authentication review request must be valid JSON."
    };
  }
}
