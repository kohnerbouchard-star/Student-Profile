  ...
  if (!control) {
    await page.waitForFunction(() => {
      ...
      return [...document.querySelectorAll("button, [role='button']")]
        .some((node) => visible(node) && String(node.textContent || "").trim() === "Players");
    }, undefined, { timeout: 120_000 });
...
console.log(JSON.stringify({ ok: true, playerLocated: evidence.playerLocated, mutation: evidence.mutation, requestCount: evidence.requests.length }));
