...
import "./admin-v2-banking.test.mjs";
import "./admin-v2-loans-api.test.mjs";
...
test("Admin v2 navigation is canonical, unique, permission-bound, and migrates source-owned routes", () => {
  ...
  assert.deepEqual(migratedRoutes.map((route) => route.id), ["overview", "players", "attendance", "market", "banking", "loans", "contracts", "business", "crafting", "store", "marketplace", "inventory", "world-management", "news-events", "messages", "progression", "settings", "logs"]);
  assert.deepEqual(
  ...
  assert.equal(isMigratedAdminRoute("progression"), true);
  ...
  assert.equal(marketplace.migration, "v2");
  assert.equal(marketplace.legacyDestination, null);

  const banking = getAdminNavigationRoute("banking");
  assert.equal(banking.groupId, "finance");
  assert.deepEqual(banking.permission.allOf, ["economy.adjust"]);
  assert.equal(banking.migration, "v2");
  assert.equal(banking.legacyDestination, null);

  const loans = getAdminNavigationRoute("loans");
  assert.equal(loans.groupId, "finance");
  assert.deepEqual(loans.permission.allOf, ["economy.adjust"]);
  assert.equal(loans.migration, "v2");
  assert.equal(loans.legacyDestination, null);
  ...
  const progression = getAdminNavigationRoute("progression");
  assert.equal(progression.label, "Progression");
  assert.equal(progression.groupId, "engagement");
  assert.deepEqual(progression.permission.allOf, ["progression.review"]);
  assert.equal(progression.migration, "v2");
  assert.equal(progression.legacyDestination, null);
...
test("Admin v2 route resolution fails closed and preserves only a validated game selector", () => {
  ...
  assert.equal(createLegacyAdminHandoffUrl("banking", locationLike), null);
  assert.equal(createLegacyAdminHandoffUrl("loans", locationLike), null);
  ...
  assert.equal(createLegacyAdminHandoffUrl("inventory", locationLike), null);
  ...
  assert.equal(createLegacyAdminHandoffUrl("progression", locationLike), null);
  ...
  assert.deepEqual(
    resolveAdminRouteBoundary({ routeId: "inventory", locationLike }),
    {
      kind: "migrated",
      route: getAdminNavigationRoute("inventory"),
      moduleKey: "inventory",
    },
  );
  assert.deepEqual(
  ...
  assert.deepEqual(
    resolveAdminRouteBoundary({ routeId: "progression", locationLike }),
    {
      kind: "migrated",
      route: getAdminNavigationRoute("progression"),
  ...
  assert.equal(resolveAdminRouteBoundary({ routeId: "progression" }).kind, "migrated");
