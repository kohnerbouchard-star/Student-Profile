...
cp package.json /tmp/seed-final-package.json

python3 - <<'PY'
from pathlib import Path
path = Path('scripts/build-seed-beta-pack.mjs')
source = path.read_text()
old = "const attendanceLateReward = Math.max(0, roundTo(attendancePresentReward * 0.5, 1, 0));"
new = "const attendanceLateReward = Math.max(0, Math.floor(attendancePresentReward * 0.5));"
if old not in source and new not in source:
    raise SystemExit('Attendance calibration patch anchor not found.')
...
const ours = JSON.parse(await readFile('/tmp/seed-final-package.json', 'utf8'));
...
Object.assign(current.scripts, {
  ...
  'preflight:seed-content:staging': 'node scripts/seed-beta-staging-preflight.mjs',
});
await writeFile('package.json', `${JSON.stringify(current, null, 2)}\n`);
NODE

git add package.json scripts/build-seed-beta-pack.mjs
...
rm -f scripts/seed-beta-final-verification-runner.sh
...
if git rev-parse -q --verify MERGE_HEAD >/dev/null; then
  git commit -m "fix(seed): synchronize and correct bounded Attendance calibration [skip ci]"
elif ! git diff --cached --quiet; then
  git commit -m "fix(seed): correct bounded Attendance calibration [skip ci]"
fi
...
run_check required seed_beta_staging_preflight npm run preflight:seed-content:staging
run_check required git_diff_check git diff --check
run_check external release_staging_preflight npm run preflight:staging
run_check external supabase_local_db_lint npm run db:lint
...
const evidence = {
  schemaVersion: 'econovaria-beta-seed-execution-evidence-v1', recordedAt: new Date().toISOString(),
  branch: 'agent/seed-content-foundation-v1', pullRequest: 163,
...
await writeFile('docs/seed-content/reviews/executable-beta-pack-readiness-v1.md', `# Executable beta seed pack readiness\n\n- PR authority: #163 on \`agent/seed-content-foundation-v1\`\n- Synchronized main: \`${process.env.MAIN_SHA}\`\n- Reconciled branch: \`${process.env.RECONCILED_SHA}\`\n- Required repository checks: ${passed}/${required.length} passed\n- Structural isolated-staging preflight: ${checks.find((entry) => entry.name === 'seed_beta_staging_preflight')?.passed ? 'passed' : 'failed'}\n- Connected isolated staging: external-blocked\n- Production touched: no\n\nThe 240-instrument bounded market, 50-item Store catalog, ten arrival calibrations, ten tutorial chains, physical-economy exploit checks, map evidence, importer dry-run, deactivation, rollback, stable-ID mapping, checksums, and authorization gates are retained under the executable pack. Full command logs are under \`docs/seed-content/reviews/executable-beta-pack-run-logs/\`.\n`);
NODE
...
if ! git diff --cached --quiet; then
  git commit -m "test(seed): retain final executable beta pack evidence [skip ci]"
  git push origin HEAD:"$BRANCH"
fi

rm -f "$ORIGINAL_APPLY" "$STATUS_FILE" /tmp/seed-final-package.json
