...
function failedRequests(startIndex = 0) {
  return evidence.requests.slice(startIndex).filter((entry) => entry.status >= 400);
}
...
async function openMessages(session) {
  ...
  const requestIndex = evidence.requests.length;
  const route = page.locator('[data-route="messages"]:visible').first();
  await route.waitFor({ state: "visible", timeout: 30_000 });
  await route.click();
  await page.waitForFunction(() => location.hash === "#messages", undefined, { timeout: 30_000 });
  await page.locator(".player-terminal-messages-page").waitFor({ state: "visible", timeout: 30_000 });
  assertNoFailedRequests(`${label} Messages route`, requestIndex);
}
...
async function createThread(sender, recipient) {
  ...
  const title = "Connected mutation verification";
  const form = page.locator('form[data-endpoint="messageThreadCreate"]');
  await form.waitFor({ state: "visible", timeout: 30_000 });
  await form.locator('[name="participantPlayerIdentifier"]').fill(recipient.playerIdentifier);
  await form.locator('[name="title"]').fill(title);
  await form.locator('[name="body"]').fill(message);
  const requestIndex = evidence.requests.length;
  const responsePromise = page.waitForResponse(
    (response) => new URL(response.url()).pathname.endsWith("/players/me/messages/threads") &&
      response.request().method() === "POST",
    { timeout: 60_000 },
  );
...
async function receiveReadAndReply(recipientSession, initial) {
  ...
  await page.getByText(initial.message, { exact: true }).waitFor({ state: "visible", timeout: 30_000 });
  evidence.messaging.recipientObservedThread = true;

  const readForm = page.locator('form[data-endpoint="messageRead"]:visible').first();
  if (await readForm.count()) {
    const requestIndex = evidence.requests.length;
    const responsePromise = page.waitForResponse(
      (response) => /\/players\/me\/messages\/threads\/thr_[0-9a-f]{32}\/read$/.test(new URL(response.url()).pathname) &&
        response.request().method() === "POST",
      { timeout: 60_000 },
    );
    await readForm.locator('button[type="submit"]').click();
    const response = await responsePromise;
    if (!response.ok()) throw new Error(`Mark message thread read returned ${response.status()}.`);
    assertNoFailedRequests("Mark message thread read", requestIndex);
    evidence.messaging.recipientMarkedRead = true;
  } else {
    throw new Error("Recipient thread did not expose its unread mutation control.");
  }

  const reply = `Connected mutation reply ${Date.now()}`;
  const sendForm = page.locator('form[data-endpoint="messageSend"]:visible').first();
  await sendForm.locator('[name="body"]').fill(reply);
  const requestIndex = evidence.requests.length;
  const responsePromise = page.waitForResponse(
    (response) => /\/players\/me\/messages\/threads\/thr_[0-9a-f]{32}\/messages$/.test(new URL(response.url()).pathname) &&
      response.request().method() === "POST",
    { timeout: 60_000 },
  );
  await sendForm.locator('button[type="submit"]').click();
  const response = await responsePromise;
  if (![200, 201].includes(response.status())) throw new Error(`Send message returned ${response.status()}.`);
  assertNoFailedRequests("Send message reply", requestIndex);
  await page.getByText(reply, { exact: true }).waitFor({ state: "visible", timeout: 30_000 });
  evidence.messaging.replySent = true;
  return reply;
}

async function verifyReply(sender, reply) {
...
try {
  ...
  await verifyReply(alpha, reply);

  if (evidence.consoleErrors.length || evidence.pageErrors.length) {
    throw new Error(`Connected mutation browser errors: ${JSON.stringify({
      consoleErrors: evidence.consoleErrors,
      pageErrors: evidence.pageErrors,
    })}`);
  }
  if (evidence.responseUuidLeak) {
    throw new Error("A connected Player mutation response exposed a raw internal UUID.");
  }
  if (!Object.values(evidence.messaging).every(Boolean)) {
    throw new Error(`Messaging mutation evidence is incomplete: ${JSON.stringify(evidence.messaging)}`);
  }
} catch (error) {
  failure = error;
  ...
  await writeFile(
    `${OUTPUT_DIR}/player-connected-mutation-browser-acceptance.json`,
...
console.log(JSON.stringify({
  ok: true,
  playersLoggedIn: evidence.playersLoggedIn,
