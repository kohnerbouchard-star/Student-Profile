...
assert.match(adminIndex, /progression-review-loader\.js/);
...
assert.match(routes, /\/players\/me\/progression\/skills\//);
assert.match(routes, /\/players\/me\/progression\/rewards\//);
assert.match(payload, /endpointKey === "progressionUnlock" \|\| endpointKey === "progressionClaim"/);
assert.doesNotMatch(JSON.stringify({ client, surface, player }), /[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}/i);
console.log("Admin and Player Progression source, privacy, accessibility, and adapter contracts passed.");
