(function initEconovariaAttendanceRewardSettingsV3() {
  "use strict";
  ...
  function renderValues() {
    ...
    const changedSummary = summary(draftWindow());
  ...
  function handleFieldChange(event) {
    const card = document.querySelector(CARD_SELECTOR);
    if (card) card.dataset.attendanceRewardDirty = "true";
  ...
  async function loadSettings() {
    ...
    try {
      ...
      state.loaded = true;
      const card = document.querySelector(CARD_SELECTOR);
      if (card?.dataset.attendanceRewardDirty !== "true") {
        card?.remove();
