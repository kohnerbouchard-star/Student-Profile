...
export function mountAdminV2({ mount, session, selectedGameId } = {}) {
  ...
  const adminTransport = createAdminBffTransport({
    selectedGameId,
    session: () => window.EconovariaAdminAuthSession?.read?.(),
  });
  const api = createAdminApiClient({ fetchImpl: adminTransport });
  const marketplaceApi = createMarketplaceApiClient({ fetchImpl: adminTransport });
  ...
  function renderRoute() {
    ...
    if (boundary.kind === "migrated") {
      ...
      content = AdminRouteBoundary({
        ...
        legacyMessage: "Overview, Store, Market, and Marketplace are native Admin v2 routes. Continue to the existing Admin for this destination without importing its generated UI into the v2 shell.",
      }).element;
