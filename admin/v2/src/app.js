...
export function mountAdminV2({ mount, session, selectedGameId } = {}) {
  ...
  const messagesApi = createMessagesAdminClient({ fetchImpl: adminBffTransport });
  ...
  function renderRoute() {
    ...
    if (boundary.kind === "migrated") {
      ...
      content = AdminRouteBoundary({
        ...
        legacyMessage: "Overview, Store, Market, and Messages are native Admin v2 routes. Continue to the existing Admin for this destination without importing its generated UI into the v2 shell.",
      }).element;
