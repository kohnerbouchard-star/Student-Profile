...
export function mountAdminV2({ mount, session, selectedGameId } = {}) {
  ...
  const bffTransport = createAdminBffTransport({
    selectedGameId,
    session: () => window.EconovariaAdminAuthSession?.read?.(),
  });
  const api = createAdminApiClient({ fetchImpl: bffTransport });
  const craftingApi = createCraftingApiClient({ fetchImpl: bffTransport });
  ...
  function renderRoute() {
    ...
    if (boundary.kind === "migrated") {
      ...
      content = AdminRouteBoundary({
        ...
        legacyMessage: "Overview, Store, Market, and Crafting are native Admin v2 routes. Continue to the existing Admin for this destination without importing its generated UI into the v2 shell.",
      }).element;
