(function initEconovariaAdminInteractionQualityFinish() {
  "use strict";
  ...
  function normalizeWriteButton(button) {
    ...
    if (completedState && button.getAttribute("aria-disabled") === "true") {
  ...
  function reconcile(root = document) {
    root.querySelectorAll?.("button[data-admin-terminal-action]").forEach(normalizeWriteButton);
  ...
  if (document.body && typeof MutationObserver === "function") {
    ...
    observer.observe(document.body, {
      attributes: true,
      attributeFilter: ["aria-disabled", "disabled", "data-admin-qol-state", "data-admin-terminal-api-state"],
      childList: true,
      characterData: true,
      subtree: true,
    });
  ...
  window.EconovariaAdminInteractionQualityFinish = {
    normalizeWriteButton,
    setScannerReady,
  };
})();