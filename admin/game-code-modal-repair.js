(function installEconovariaGameCodeModalRepair() {
  ...
  function ensureGenerateControl() {
    ...
    const code = normalizedText(codeLabel?.textContent).toUpperCase();
    ...
    resetButton.title = hasReadableCode
      ...
      : "Generate a readable code for this game.";
  ...
  function scheduleRepair() {
    ...
    pendingTimers = RETRY_DELAYS_MS.map((delay) =>
      window.setTimeout(ensureGenerateControl, delay)
    );
  }

  document.addEventListener("click", (event) => {
    if (event.target?.closest?.(SHARE_TRIGGER_SELECTOR)) scheduleRepair();
  }, true);
