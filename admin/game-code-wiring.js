(function initEconovariaGameCodeWiring() {
  ...
  function ensureActionIntegrityToken() {
    ...
    const meta = document.querySelector(
      'meta[name="econovaria-csrf-token"]',
    );
  ...
  function syncModelCode(code) {
    ...
    const nextModel = {
      ...
      activeGame: patchGameRecord(
        model.activeGame || { id: gameId, gameId },
        gameId,
        normalized,
      ),
    };
    ...
    if (Array.isArray(model.games)) {
      nextModel.games = model.games.map((game) =>
        patchGameRecord(game, gameId, normalized)
      );
    }
    if (Array.isArray(model.activeGameSessions)) {
      nextModel.activeGameSessions = model.activeGameSessions.map((game) =>
        patchGameRecord(game, gameId, normalized)
      );
    }
    ...
    const configured = String(
      window.ECONOVARIA_PLAYER_APP_URL ||
        document.querySelector(
          'meta[name="econovaria-player-app-url"]',
        )?.content ||
        "/play",
    ).trim();
  ...
  function applyCodeToRenderedShareSurface(code) {
    const normalized = normalizeCode(code);
    if (!normalized) return;
    const gameId = selectedGameId();

    document
      .querySelectorAll(".admin-terminal-side-code-expanded strong")
      .forEach((node) => setText(node, normalized));
    document
      .querySelectorAll("[data-admin-terminal-share-button]")
      .forEach((button) => {
        button.dataset.gameCode = normalized;
        button.setAttribute(
          "aria-label",
          `Share game code ${normalized}`,
        );
      });
    document.querySelectorAll("[data-game-id]").forEach((node) => {
      if (String(node.dataset.gameId || "") === gameId) {
        node.dataset.gameCode = normalized;
      }
    });
    ...
    setText(
      modal.querySelector(".admin-terminal-share-modal-code strong"),
      normalized,
    );
    ...
    const studentInput = modal.querySelector(
      'input[id*="share-student-link"]',
    );
    const adminInput = modal.querySelector(
      'input[id*="share-admin-link"]',
    );
    const invite = modal.querySelector('textarea[id*="share-invite"]');
    const gameName = String(
      feature()?.currentModel?.gameName || "Eco Novaria",
    ).trim() || "Eco Novaria";
  ...
  function setInlineMessage(modal, message, tone = "") {
    ...
    if (!node) {
      ...
      modal
        .querySelector(".admin-terminal-share-modal-code")
        ?.appendChild(node);
    }
  ...
  function decorateShareModal() {
    ...
    const codeSection = modal.querySelector(
      ".admin-terminal-share-modal-code",
    );
    if (!codeSection) return false;

    const code = currentCode();
    if (code) applyCodeToRenderedShareSurface(code);
    ...
    if (!resetButton) {
      ...
      resetButton.dataset.adminTerminalAction = RESET_ACTION;
  ...
  function scheduleShareModalDecoration() {
  ...
  function installStyles() {
    if (
      document.querySelector(
        "style[data-econovaria-game-code-wiring]",
      )
    ) return;
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      if (actionName === RESET_ACTION) {
        event.preventDefault();
        event.stopImmediatePropagation();
        void resetGameCode(action);
        return;
      }

      if (SHARE_ACTIONS.has(actionName)) {
        scheduleShareModalDecoration();
      }
    },
    false,
  );
